...
def test_log_stats_basic_functionality():
    ...
    if len(result) > 0:
        ...
        print(
            f"  Sample: time_bin={row['time_bin']}, process_id={row['process_id']}, level={row['level']}, target={row['target']}, count={row['count']}"
        )
        print("✅ Schema validation passed")
    else:
        pytest.fail(
            "No log_stats records found in time range - view may not be materialized"
        )
...
def test_log_stats_process_filtering():
    ...
    if len(sample_result) == 0:
        pytest.fail(
            "No sample data available for filtering test - insufficient test data"
        )
    ...
    if len(filter_result) == 0:
        pytest.fail(
            f"No results from filtering by process_id='{sample_process}' and target='{sample_target}' - filtering may be too restrictive or data too sparse"
        )
...
def test_log_stats_performance():
    ...
    if len(result) == 0:
        pytest.fail(
            "No aggregated records returned for performance test - insufficient test data"
        )
