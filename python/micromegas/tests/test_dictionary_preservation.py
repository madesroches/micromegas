...
def test_properties_to_dict_with_default_client():
    ...
    try:
        ...
        sql = (
            "SELECT properties_to_dict(properties) as dict_props FROM measures LIMIT 5"
        )
        ...
        assert not pa.types.is_dictionary(
            dict_column.type
...
def test_properties_to_dict_with_dictionary_client():
    ...
    try:
        ...
        if pa.types.is_dictionary(dict_column.type):
            ...
            print(
                "   ⚠️  Dictionary not preserved - may be expected if server feature is disabled"
            )
...
def test_properties_length_compatibility():
    ...
    try:
        ...
        print(
            f"   properties_length(properties_to_dict(properties)) results: {df_dict['len'].tolist()}"
        )

        # Results should be identical
        assert df_array["len"].equals(
            df_dict["len"]
...
def test_pandas_conversion_with_dictionary_preservation():
    ...
    try:
        ...
        sql = (
            "SELECT properties_to_dict(properties) as dict_props FROM measures LIMIT 3"
        )
        ...
        print(
            f"   Arrow preserves dictionary: {pa.types.is_dictionary(dict_column_arrow.type)}"
        )
        print(
            f"   Pandas receives regular type: {not str(dict_column_pandas_source).startswith('dictionary')}"
        )
...
def test_query_arrow_method():
    ...
    try:
        sql = (
            "SELECT properties_to_dict(properties) as dict_props FROM measures LIMIT 2"
        )
