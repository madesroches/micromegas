...
def test_perfetto_trace_chunks_integration():
    ...
    if not process_info:
        pytest.skip("No test process found - run telemetry generator to create test data")
    ...
    print(f"  Streams: {process_info['stream_count']}, Blocks: {process_info['block_count']}")
    
    # Check data availability
    data_availability = check_process_data_availability(process_id, start_time, end_time)
    print(f"  Thread streams: {data_availability['thread_streams']}")
    print(f"  Async events: {data_availability['async_events']}")
    
    if data_availability['thread_streams'] == 0 and data_availability['async_events'] == 0:
    ...
    assert len(span_type_results) > 0, "At least one span type should generate traces successfully"
    ...
    if 'both' in span_type_results:
        ...
        assert len(trace_bytes) > 100, "Complete trace should be substantial (>100 bytes)"
