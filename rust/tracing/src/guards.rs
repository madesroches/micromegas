...
impl AsyncSpanGuard {
    pub fn new(span_desc: &'static SpanMetadata) -> Self {
        let span_id = on_begin_async_scope(span_desc, 0, 0);
...
impl Drop for AsyncSpanGuard {
    fn drop(&mut self) {
        on_end_async_scope(self.span_id, 0, self.span_desc, 0);
    }
}
...
impl AsyncNamedSpanGuard {
    pub fn new(span_location: &'static SpanLocation, name: &'static str) -> Self {
        let span_id = on_begin_async_named_scope(span_location, name, 0, 0);
        Self {
            span_location,
            name,
            span_id,
...
impl Drop for AsyncNamedSpanGuard {
    fn drop(&mut self) {
        on_end_async_named_scope(self.span_id, 0, self.span_location, self.name, 0);
    }
}
