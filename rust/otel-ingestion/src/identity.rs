...
///
/// Every process gets an audience, always (#1482 §0), so this struct deliberately has no
/// `Default`: `<&str>::default()` is `""`, and an empty audience folded silently into every id
/// is exactly the kind of silent default `CLAUDE.md`'s "compiler enumerates every call site" rule
/// exists to prevent. Every production construction site is a full struct literal.
...
///
/// `ctx.audience` (AbAC Stage 5, #1373, §4) domain-separates the hash: it derives a per-audience
/// namespace UUID (`NS_OTEL_PROCESS_V1` salted with the audience) and hashes the joined field key
/// under *that* namespace, rather than appending the audience string onto the joined key with the
/// same `\x1F` separator that joins the 31 fields above. Concatenating into the shared-separator
...
/// trims and lower-cases, it does not escape `\x1F`. This is what stops two audiences sending
/// identical resource attributes (the same containerized app in two tenants, a degenerate
/// resource, a CloudWatch namespace) from silently colliding on one `process_id`, including via a
...
pub fn process_id_from_resource(resource: Option<&Resource>, ctx: IdentityContext) -> Uuid {
    ...
    let audience_ns = Uuid::new_v5(&NS_OTEL_PROCESS_V1, ctx.audience.as_bytes());
