...
use analytics_web_srv::ingestion_keys::{IngestionKeysState, ingestion_keys_router};
...
// ---------------------------------------------------------------------------
// NotConfigured 503 — `IngestionKeysState { pool: None }` never touches the
...
async fn mint_503_when_pool_unconfigured() {
    let app = build_handler_router_with_user(IngestionKeysState { pool: None }, admin_user());
    let response = app
        .oneshot(post_request("/api/ingestion-api-keys", r#"{"name": "x"}"#))
        .await
        .expect("call service");
