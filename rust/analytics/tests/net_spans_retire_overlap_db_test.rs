...
//! `view_set_name = "net_spans"` with synthetic `lakehouse_partitions` rows covers every arm the
//! net_spans write path depends on -- the overlap arm, the containment arm, the degenerate-new-
//! range third arm, and the same-run identity exclusion. The block-grouping half of the change is
//! already covered view-agnostically by `jit_partition_grouping_tests.rs` and
//! `jit_partition_bounds_tests.rs`.
//!
...
/// The degenerate-new-range third arm: when the new partition's range is degenerate
/// (`begin_insert_time == end_insert_time`, i.e. every block in it shares one insert time), the
/// first two arms both miss a wider existing partition that merely contains that instant --
/// `tstzrange(t, t)` is Postgres's empty range so `&&` is vacuously false, and the containment arm
/// degenerates to an exact match. The third arm's `begin <= $3 AND end > $3` covers it.
#[ignore]
#[tokio::test]
...
/// they would otherwise match one of the three arms. `net_spans_view::jit_update` threads a fresh
/// `same_run_ranges` accumulator through its loop for exactly this reason.
#[ignore]
#[tokio::test]
...
async fn net_spans_stale_degenerate_predecessor_is_retired() -> Result<()> {
    ...
    // same instant -- the containment arm must match the degenerate predecessor.
    retire(
        &lake,
        &view_instance_id,
        t0,
