...
async fn migration_enforces_partition_overlap_constraint() -> Result<()> {
    ...
    insert_test_partition(&pool, "global", "a.parquet", t0, t0 + hour)
        .await
        .with_context(|| "inserting base partition")?;

    // Adjacent partition sharing a boundary: tstzrange is '[)', must not conflict.
    insert_test_partition(&pool, "global", "b.parquet", t0 + hour, t0 + hour * 2)
        .await
        .with_context(|| "inserting adjacent partition")?;

    // Same range, different view_instance_id: must not conflict.
    insert_test_partition(&pool, "other_instance", "c.parquet", t0, t0 + hour)
        .await
        .with_context(|| "inserting same-range partition for another instance")?;

    // Overlapping partition: must be rejected by the exclusion constraint, reported through
    // the structured constraint name (what insert_partition_transaction matches on).
    ...
    sqlx::query("DELETE FROM lakehouse_partitions WHERE view_instance_id = 'global';")
        .execute(&mut *tr)
        .await?;
    insert_test_partition(&mut *tr, "global", "merged.parquet", t0, t0 + hour * 2)
        .await
    ...
    // overlapping pair, and verify the migration refuses to add the constraint, naming the
    // conflict instead of failing with a raw constraint error.
    sqlx::query(
    ...
    .execute(&pool)
    .await
    .with_context(|| "planting overlapping pair for detect-then-fail")?;
    ...
    // Clean up the planted overlap and verify the migration then completes.
    sqlx::query("DELETE FROM lakehouse_partitions WHERE file_path IN ('x.parquet', 'y.parquet');")
        .execute(&pool)
        .await?;
    migrate_lakehouse(pool.clone())
        .await
        .with_context(|| "re-running migration after retiring the overlap")?;
    Ok(())
}
