...
use opentelemetry_proto::tonic::metrics::v1::{
    DataPointFlags, Metric, ResourceMetrics, ScopeMetrics, Summary, SummaryDataPoint, metric,
...
/// Reads a string-dictionary column (`name`, `unit`, ...) out as one `String` per row.
fn dict_strings(batch: &datafusion::arrow::record_batch::RecordBatch, col: &str) -> Vec<String> {
    let dict = batch
        .column_by_name(col)
        ...
        >>()
        .unwrap_or_else(|| panic!("{col} is a dictionary array"));
    let values = dict
        .values()
        .as_any()
...
fn times_nanos(batch: &datafusion::arrow::record_batch::RecordBatch) -> Vec<i64> {
    let col = batch
        .column_by_name("time")
        .expect("time column")
        .as_any()
        .downcast_ref::<datafusion::arrow::array::TimestampNanosecondArray>()
        .expect("time is TimestampNanosecond");
...
async fn summary_fans_out_to_four_suffixed_rows_and_drops_non_min_max_quantiles() {
    ...
    let rows_time_range = row_set.rows_time_range;
    let batch = row_set.rows;

    assert_eq!(batch.num_rows(), 4, "count/sum/min/max only, 0.9 dropped");

    // All four rows carry the source data point's timestamp, and the row set's time
    // range collapses to that instant.
    assert_eq!(times_nanos(&batch), vec![1_000; 4]);
    assert_eq!(rows_time_range.begin.timestamp_nanos_opt(), Some(1_000));
    assert_eq!(rows_time_range.end.timestamp_nanos_opt(), Some(1_000));

    let mut rows: Vec<(String, String, f64)> = dict_strings(&batch, "name")
        .into_iter()
        .zip(dict_strings(&batch, "unit"))
...
async fn summary_without_min_max_quantiles_still_produces_count_and_sum() {
    ...
    let mut got_names = dict_strings(&batch, "name");
    got_names.sort();
    assert_eq!(
        got_names,
        vec![
...
async fn summary_with_duplicate_min_max_quantiles_emits_one_row_each() {
    ...
    let mut rows: Vec<(String, f64)> = dict_strings(&batch, "name")
        .into_iter()
        .zip(values_f64(&batch))
        .collect();
    rows.sort_by(|a, b| a.0.cmp(&b.0));
...
/// A point flagged `NO_RECORDED_VALUE` is a gap marker, not a sample: materializing it
/// would emit `_count = 0` / `_sum = 0` rows that skew `avg`/`min` aggregations.
#[tokio::test]
async fn summary_data_point_flagged_no_recorded_value_produces_no_rows() {
    let dp = SummaryDataPoint {
        attributes: vec![],
        start_time_unix_nano: 0,
        time_unix_nano: 1_000,
    ...
    assert!(
        result.is_none(),
        "NO_RECORDED_VALUE data point should yield no rows at all"
    );
}

/// The counterpart to the flagged case: an *unflagged* `count = 0`/`sum = 0` point is a real
/// observation ("nothing happened in this window"), distinct from missing telemetry, so it
/// must still materialize.
#[tokio::test]
async fn summary_data_point_with_genuine_zero_count_still_produces_rows() {
    let dp = SummaryDataPoint {
        attributes: vec![],
        start_time_unix_nano: 0,
        time_unix_nano: 1_000,
    ...
    let result = processor
        ...
        .await
        .expect("process must not fail");
    let batch = result.expect("expected Some(row_set)").rows;
    ...
    assert_eq!(
        rows,
        vec![
            ("CPUUtilization_count".to_string(), 0.0),
            ("CPUUtilization_sum".to_string(), 0.0),
        ]
    );
}
