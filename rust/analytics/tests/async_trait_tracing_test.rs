...
impl ComplexService for ComplexServiceImpl {
    #[span_fn]
    async fn complex_method(&self, data: &[u8], _options: HashMap<String, String>) -> Result<Vec<u8>, String> {
        Ok(data.to_vec())
    }
}
...
fn test_async_trait_span_fn_comprehensive() {
    ...
    runtime.block_on(async {
        tokio::task::spawn(async {
            ...
            let result4 = complex_service.complex_method(b"complex_data", options).await;
...
fn test_async_trait_span_fn_current_behavior() {
    ...
    if total_events == 6 {
        ...
        println!("UNEXPECTED BEHAVIOR: Got {} events, need to investigate", total_events);
...
fn test_simple_service_process_events() {
    ...
    if total_events == 2 {
        ...
        println!("? UNEXPECTED: Got {} events, need investigation", total_events);
    }
...
fn test_comparison_async_trait_vs_regular() {
    ...
    if total_events == 4 {
        println!("✓ PERFECT: Both async trait method and regular async function generated identical event counts!");
        ...
        println!("  CONCLUSION: #[span_fn] works identically for async traits and regular async functions");
    } else {
        println!("? UNEXPECTED: Expected 4 events total, got {}", total_events);
    }
