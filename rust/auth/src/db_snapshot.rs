...
//! per-key `moka` cache: the whole table is small enough to hold as one snapshot, so `moka`'s
...
/// What a [`SnapshotSource`] needs from a specific store: how to fetch a fresh snapshot from the
/// pool, and the name/metric to report on a refresh failure.
#[async_trait]
pub trait SnapshotLoader: Send + Sync + 'static {
    /// The loaded, ready-to-serve snapshot type (e.g. `AudienceGrants`, `GroupGraph`).
    type Snapshot: Send + Sync;

    /// Human-readable store name used in error/log text (e.g. "audience grant store", "group
    /// store").
    const NAME: &'static str;

    /// Queries the whole table(s) and builds a fresh snapshot.
    async fn fetch(pool: &PgPool) -> Result<Self::Snapshot>;

    /// Emits this store's own refresh-error metric (`imetric!()` with its own literal name) --
    /// called by [`SnapshotSource::current`] at both failure sites.
...
struct Snapshot<T> {
    ...
    /// reported in the refresh-failure log/error context ("how stale is the view right now"),
    /// independent of `fetched_at`; it is not itself emitted as a metric.
    loaded_at: Instant,
...
/// The whole-table snapshot cache described in the module doc comment, generic over a
/// [`SnapshotLoader`].
pub struct SnapshotSource<L: SnapshotLoader> {
    pool: PgPool,
    ttl: Duration,
    snapshot: tokio::sync::RwLock<Option<Snapshot<L::Snapshot>>>,
...
impl<L: SnapshotLoader> std::fmt::Debug for SnapshotSource<L> {
...
impl<L: SnapshotLoader> SnapshotSource<L> {
    ...
    fn throttled_error(&self, last_attempt_millis: i64) -> anyhow::Error {
        ...
        ProviderUnavailable(anyhow!(
            "{} unavailable; cold-start retry throttled, retry available in {} seconds",
            L::NAME,
            retry_in.as_secs()
        ))
        .into()
    ...
    /// Returns the current snapshot, refreshing it first if stale.
    ///
    ...
    /// load has succeeded, a later refresh failure is logged + counted (via
    /// [`SnapshotLoader::count_refresh_error`]) and the last good snapshot keeps serving,
    /// unbounded -- this store has no per-item TTL eviction to fall back on the way
    ...
    pub async fn current(&self) -> Result<Arc<L::Snapshot>> {
        ...
        if had_snapshot {
            ...
            // cheap (the whole point of "the table is small").
            match L::fetch(&self.pool).await {
                Ok(value) => {
                ...
                Err(err) => {
                    L::count_refresh_error();
                    let mut guard = self.snapshot.write().await;
                    match guard.as_mut() {
                        Some(snap) => {
                            let age_secs = snap.loaded_at.elapsed().as_secs();
                            micromegas_tracing::error!(
                                "{} refresh failed, serving stale snapshot (age={age_secs}s): \
                                 {err:#}",
                                L::NAME
                            );
                            snap.fetched_at = Instant::now();
                            Ok(Arc::clone(&snap.value))
                        }
            ...
            match L::fetch(&self.pool).await {
                ...
                Err(err) => {
                    L::count_refresh_error();
                    micromegas_tracing::error!("{} cold-start load failed: {err:#}", L::NAME);
