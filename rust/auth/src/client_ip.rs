//! Resolves the client IP address from request headers/extensions.
//!
//! Moved down from `rust/public/src/servers/http_utils.rs::get_client_ip` so every
//! `AuthProvider` (not just audit logging) can consult the same resolution: `micromegas-auth`
//! cannot depend on the `public` crate (it's the other way around), so this logic has to live
//! here for `IpAllowlist::allows` to use it. `get_client_ip` becomes a thin formatting wrapper
//! over [`resolve_client_ip`].

use std::net::IpAddr;

/// Resolves the client IP address from HTTP headers and extensions.
///
...
///
/// The resolved address is always run through [`IpAddr::to_canonical`] before being returned, so
/// an IPv4-mapped IPv6 form (`::ffff:a.b.c.d`) arriving from a dual-stack listener unmaps to
/// `a.b.c.d` here -- the single normalization point, independent of listener configuration --
/// before an [`crate::ip_allowlist::IpAllowlist`] ever sees the address, so an allowlist entry of
/// `a.b.c.d/32` matches either arrival form. This is a deliberate behavior change from the
/// `get_client_ip` this was moved from, which returned the un-normalized form; it also normalizes
/// the form logged for audit purposes.
///
/// Returns `None` if no IP can be extracted.
pub fn resolve_client_ip(
    ...
    if let Some(forwarded_for) = headers.get_all("x-forwarded-for").iter().next_back()
        ...
        && let Ok(client_ip) = candidate.parse::<IpAddr>()
    {
        return Some(client_ip.to_canonical());
    }
    ...
    {
        return Some(connect_info.0.ip().to_canonical());
    }

    if let Some(remote_addr) = extensions.get::<std::net::SocketAddr>() {
        return Some(remote_addr.ip().to_canonical());
    }
