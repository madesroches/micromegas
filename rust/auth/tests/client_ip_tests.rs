// Unit tests for `micromegas_auth::client_ip::resolve_client_ip`'s selection,
// fallback, and anti-spoofing rules: picks the *rightmost* `X-Forwarded-For`
// entry of the *last* header field line. Moved from
// `rust/public/tests/http_utils_tests.rs`, retargeted at `resolve_client_ip` and
// asserting `Option<IpAddr>` instead of a formatted string.
...
fn multi_entry_chain_returns_rightmost_entry_trimmed() {
    ...
    let resolved = resolve_client_ip(&headers, &Extensions::new());
...
fn single_entry_chain_returns_that_entry() {
    ...
    assert_eq!(resolved, Some(ip("203.0.113.1")));
}
...
fn no_x_forwarded_for_falls_back_to_x_real_ip() {
    ...
    assert_eq!(resolved, Some(ip("203.0.113.42")));
}
...
fn no_headers_falls_back_to_connect_info_extension() {
    ...
    let resolved = resolve_client_ip(&headers, &extensions);

    assert_eq!(resolved, Some(ip("192.168.1.100")));
}
...
#[test]
fn non_ip_x_forwarded_for_with_no_other_source_returns_none() {
    let mut headers = HeaderMap::new();
...
/// An IPv4-mapped IPv6 socket address (`::ffff:a.b.c.d`), as a dual-stack listener might hand
/// back, resolves to the canonical IPv4 form -- the new normalization `resolve_client_ip` adds
/// over the `get_client_ip` it was moved from.
#[test]
fn ipv4_mapped_ipv6_socket_addr_resolves_to_canonical_ipv4() {
    let headers = HeaderMap::new();
    let mut extensions = Extensions::new();
    extensions.insert(socket_addr("[::ffff:203.0.113.7]:9000"));
