...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..args.async_tasks {
        let handle = tokio::spawn(async move {
            multi_threaded_async_work(i).await
        });
        handles.push(handle);
    }
    
    // Create concurrent async tasks with work-stealing potential (using args.threads)
    let operations = ["database_query", "api_call", "file_processing", "cache_lookup", "network_request", "disk_io"];
    ...
    info!("running for {} seconds to generate continuous telemetry", args.duration);
