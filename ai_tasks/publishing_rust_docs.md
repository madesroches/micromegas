...
*   **Rust Documentation Root 404 (Persistent):** Despite previous attempts, `https://<username>.github.io/<repository-name>/rust/` still returns a 404. This is due to a misunderstanding of `cargo doc`'s output location in a workspace. When `cargo doc` is run with `working-directory: rust/`, it correctly places its output in `rust/target/doc/` (relative to the project root).
    *   **Resolution:** The `cp` command for Rust documentation will be changed back to `cp -r rust/target/doc/* public_docs/rust/`. This will ensure the entire generated documentation, including the `micromegas` subdirectory and all shared assets, is correctly placed in the deployed site.
