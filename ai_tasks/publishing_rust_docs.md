...
*   **Rust Documentation Root 404 (Persistent):** Despite previous attempts, `https://<username>.github.io/<repository-name>/rust/` still returns a 404. This is due to a misunderstanding of `cargo doc`'s output location in a workspace. Even with `working-directory: rust/`, `cargo doc` places its output in the project root's `target/doc/` directory.
    *   **Resolution:** The `cp` command for Rust documentation will be changed to copy from `target/doc/` (relative to the project root) into `public_docs/rust/`. This will ensure the entire generated documentation, including the `micromegas` subdirectory and all shared assets, is correctly placed in the deployed site.
