...
generators, per `tasks/jit_batched_block_queries_plan.md`'s *Sort elision* follow-up) — so they
...
(a pure function of `insert_time`; see `tasks/jit_batched_block_queries_plan.md`), so single-column
...
deferred JIT-consumer-trust follow-up (`tasks/jit_batched_block_queries_plan.md`'s *Sort elision*
...
- **`ORDER BY` + declared scan ordering vs. `ORDER BY` alone.** An `ORDER BY` with no declared
  ...
  every merge — the exact problem `tasks/jit_batched_block_queries_plan.md` was written to avoid on the
...
- **Not declaring the JIT-consumer-side `insert_time` scan ordering in this plan.** Doing so
  ...
  tie-atomic, soft-cap segmentation (`tasks/jit_batched_block_queries_plan.md`). This deferral is
  tracked as its own open item in `tasks/jit_batched_block_queries_plan.md`'s Follow-ups (see *Sort
...
- **Why the recorded `sort_order` is single-column `[insert_time]`, not `[insert_time, block_id]`.**
  ...
  JIT segmenter in `tasks/jit_batched_block_queries_plan.md` — no longer relies on a stored total order:
...
`tasks/jit_batched_block_queries_plan.md`'s segmenter design.)
