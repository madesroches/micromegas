...
**Peer warnings on install (informational, not a blocker)**: `@grafana/plugin-ui@0.16.1` declares `engines: {"node": ">=24", "yarn": ">=4.16.0"}` (verified via `npm view @grafana/plugin-ui@0.16.1 engines --json`), but a clean `yarn install` with this repo's Yarn `4.14.1` prints no engines warning at all — Yarn's engines check only fires when `engineStrict` (or an equivalent) is set, and neither `grafana/.yarnrc.yml` nor the root `.yarnrc.yml` sets one. What `yarn install` actually prints instead is a pair of `YN0060` peer warnings: `react` and `react-dom` (both `18.3.1` here) don't satisfy what `react-virtualized-auto-sizer@1.0.6` (pinned as a transitive dependency inside `@grafana/plugin-ui`) requests. These are non-blocking and don't fail the install or CI. Bumping Node/yarn is out of scope for this dependency-bump plan; this is noted here so the warnings aren't mistaken for a regression.
...
1. **Bump frontend SDK dependencies**
   ...
   - Run `yarn install` from `grafana/` and commit the updated lockfile entries (root `yarn.lock`). Expect non-blocking `YN0060` peer warnings for `react`/`react-dom` not satisfying `react-virtualized-auto-sizer@1.0.6` (pinned inside `@grafana/plugin-ui`; see Dependency changes) — this doesn't fail the install. (An unrelated `@openfeature/core` `YN0002` peer warning from `@openfeature/web-sdk` — a `@grafana/runtime` dependency, not introduced by this plan — is resolved by declaring `@openfeature/core` as an explicit dependency alongside it.)
...
3. **Refresh generated scaffolding — diffed against a real reference scaffold, nothing hand-ported**
   - `npx @grafana/create-plugin@latest generate`'s prompts (`pluginType`, `hasBackend`, `pluginName`, `orgName`) accept CLI flags directly (see `dist/commands/generate/prompt-user.js`), so a non-interactive reference scaffold could be generated after all: `generate --pluginType=datasource --hasBackend=true --pluginName="Micromegas FlightSQL" --orgName=micromegas`.
   - Diffed the result's `.config/`, `tsconfig.json`, and `eslint.config.mjs` against this repo's equivalents. As the Caveat above anticipated, essentially the entire diff is tied to the reference scaffold's Grafana 13.x architecture, not to anything this 12.x bump needs:
     - `webpack.config.ts` is a ground-up rewrite for 13.x (new `bundler/` helper dir, `SubresourceIntegrityPlugin`, a virtual `grafana-public-path` module, `BuildModeWebpackPlugin`, `TerserPlugin` options) — none of it applicable to a 12.x plugin.
     - `eslint.config.mjs` now imports a generated `.config/eslint.config.mjs` base (via `defineConfig`) instead of `@grafana/eslint-config/flat.js` directly — this is `@grafana/eslint-config@10`'s restructuring, which this plan already deferred (see Dependency changes: staying on `^9.0.0` because `10.0.0` drops the `flat.js` export this repo's config imports).
     - New top-level files (`entrypoint.sh`, `docker-compose-base.yaml`, a `supervisord` dir) are the 13.x backend-container split, unrelated to a dependency bump.
     - `jest.config.js`/`jest-setup.js`/`.config/jest/utils.js` differ only in a doc-link comment, `sourceMaps: 'inline'` vs `true`, a `watchPathIgnorePatterns` addition, a `TextEncoder`/`TextDecoder` polyfill, and a much longer `grafanaESModules` transform allowlist (`d3`, `marked`, `memoize`, `@grafana/schema`, etc.) — packages this plugin doesn't depend on. `yarn test:ci` passes without any of these, so none are needed for this bump; they're a possible future hardening item, not a blocker.
   - One genuine confirmation came out of the diff: the 13.x reference's own generated `.config/tsconfig.json` sets its `ts-node` override to `module`/`moduleResolution: "nodenext"`, `target: "es2022"` — i.e. upstream independently landed on the exact same fix for the `@grafana/tsconfig` `moduleResolution: "bundler"` conflict that commit `b135778f6` applied here (in the non-generated root `grafana/tsconfig.json`, since `.config/tsconfig.json` is DO-NOT-EDIT and this repo has no `.cprc.json` for `create-plugin update` to key off). No further scaffolding change is needed.
