...
- `retire_partition_in_transaction` (:79–159) runs two hash-blind statements against
  `lakehouse_partitions`: a `SELECT file_path, file_size ... fetch_optional` (:88–100) and a
...
- The only post-delete check is `delete_result.rows_affected() == 0` (:147). A two-row delete
  reports `SUCCESS:`.
- `invoke_async_with_args` (:187) hard-asserts `args.len() != 4`, downcasts the four arrays,
  and drives one transaction across the whole batch, rolling back if any row errored.

### Why two rows can share the key

`rust/analytics/src/lakehouse/migration.rs:510-515` documents cross-schema coexistence as legal:
...
- **0 rows** → bail with today's message plus the hash named:
  `"Partition not found: <view_set>/<instance> [<begin>, <end>) schema_hash=<hex>"`.
- **>1 rows** → since the `SELECT` already pins `file_schema_hash = $5`, this is only reachable
  ...
  (`write_partition.rs:205-215` documents `begin_insert == end_insert` for JIT partitions). Bail
  with `"Ambiguous match: <n> rows for <view_set>/<instance> [<begin>, <end>)
  schema_hash=<hex>, file_paths=[<path1>, <path2>, ...]; use retire_partition_by_file when
  file_paths are distinct and non-NULL"`. When they carry distinct non-NULL `file_path`s,
  `retire_partition_by_file` is the disambiguating escape hatch, since a fifth argument cannot
  help here — the rows already share it. Two colliding rows that are both empty partitions
  (`file_path IS NULL`, per `write_partition.rs:843-850` and `:897-902`) have no `file_path` to
  key on either, so they remain unretirable through any UDF; see `## Decisions`.
- **1 row** → `add_file_for_cleanup` for its `file_path` (unchanged), then `DELETE` keyed on all
  five columns, including `file_schema_hash = $5`.

The `file_schema_hash = $5` predicate on the `DELETE` is what closes the READ COMMITTED race: even
if a concurrent writer commits a new-schema partition on the same range between the two
statements, the `DELETE` cannot reach it. The post-delete check tightens from
`rows_affected() == 0` to `rows_affected() != 1`, so any surviving surprise aborts the batch and
...
1. **Signature and argument plumbing** (`retire_partition_by_metadata_udf.rs`)
   ...
   - Hex-format `args[4]` into the `SUCCESS:` message's `schema_hash=<hex>` suffix (:270–272).

2. **Fail-closed resolution** (same file, `retire_partition_in_transaction`)
   - Add the `file_schema_hash: &[u8]` parameter.
   - Add a plain `file_schema_hash = $5` predicate to the `SELECT`; switch to `fetch_all`.
   - Add the empty / ambiguous / single-row branches described above.
   - Key the `DELETE` on all five columns, including `file_schema_hash = $5`; assert
     `rows_affected() == 1`.
...
4. **Tests** — implement the Rust live-DB, Rust offline, and Python cases described in
   `## Testing Strategy`.

5. **Docs** — `mkdocs/docs/admin/functions-reference.md`,
   `mkdocs/docs/query-guide/functions-reference.md`.

6. **CHANGELOG** — entry under `## Unreleased`, filed as a **Minor breaking change** per the
...
- The fifth argument is required, accepting the resulting SQL-surface break because this is an
  admin-only API.
- The `Binary` slot accepts only `Binary` or `Null`; anything else — including `Utf8`,
  `LargeBinary`, and `Dictionary(_, Binary)` — fails loudly at planning.
- An ambiguous match errors out rather than deleting every matching row — deleting all would fix
  the storage leak but keep the data loss.
- Two colliding rows that are both empty partitions (`file_path IS NULL`) remain unretirable
  through any UDF, since `retire_partition_by_file` has no `file_path` to key on either; accepted
  as a residual limitation.

## Documentation

- `mkdocs/docs/admin/functions-reference.md:157` — update the `###` heading to the five-argument
  signature, document the required parameter, document the ambiguity error under **Safety**,
  update the **Usage** block (:171–178) and the first **Example** block (:191–198) to the
  five-argument form, and change the "Batch retire incompatible partitions" example (:204) to
  pass `p.file_schema_hash`. Changing the heading changes its slug, so update the cross-link in
...
- `mkdocs/docs/admin/functions-reference.md:180-182` — update the **Returns** block, which quotes
  the success/failure strings verbatim, to match the `schema_hash=<hex>` suffix from Design §3;
  line 182's "Partition not found" failure string also changes, to the amended string from Design
  §2; and add the new ambiguity error string from Design §2.
