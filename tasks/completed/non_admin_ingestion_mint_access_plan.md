...
- `hooks/useMyAudiences.ts` (new): the `me`/`selfServiceOff`/`authDisabled`/`myAudiencesError` state and `loadMyAudiences` callback currently inlined at `AudienceAccessPage.tsx:538-546,573-595`, returning `{ me, selfServiceOff, authDisabled, error, reload }`. `AudienceAccessPage.tsx` switches to this hook. `/visible` and `/my-audiences` share the same `--disable-auth` gate (per the existing code's own comment), so they always fail together — the `listQuery`-completion effect's own `listQuery.error.code === 'AUTH_DISABLED'` check is redundant with the hook's `authDisabled`, which already covers it. That effect keeps only its `setListError(...)` call; the now-orphaned `setAuthDisabled(true)` line is dropped, since the hook owns `authDisabled` and no longer exposes a setter.
...
- `AudienceAccessPage.tsx`'s breadcrumb currently renders the "Admin /" link only when `isAdmin` (lines 782-789). Since `/admin` becomes viewable by every authenticated user, drop that `isAdmin &&` gate so the link renders unconditionally, matching `ApiKeysAdminPage.tsx:168`'s unconditional breadcrumb.
...
- Call `usePageTitle('Ingestion API Keys')` once in this wrapper, before branching on role, so both the admin and non-admin branches set the document title — mirroring `AudienceAccessPage.tsx`'s single unconditional `usePageTitle('Audience Access')` call ahead of its own role branch. On the admin branch this duplicates `ApiKeysAdminPage.tsx`'s own `usePageTitle(config.title)` call with the same string; the hook is idempotent on identical input, so the redundant call is harmless and `ApiKeysAdminPage.tsx` (shared with `/admin/analytics-keys`) stays untouched.
...
3. `analytics-web-app/src/routes/AudienceAccessPage.tsx`: switch to the new hook and component. In the `listQuery` completion effect, drop the now-orphaned `setAuthDisabled(true)` line (the hook no longer exposes a setter and already covers this case, since `/visible` and `/my-audiences` share one `--disable-auth` gate) and keep only its `setListError(...)` call. Drop the `isAdmin &&` gate around the "Admin" breadcrumb link (lines 782-789) so it renders for every authenticated user, matching `ApiKeysAdminPage.tsx`'s unconditional breadcrumb. Verify behavior and existing tests are otherwise unaffected.
4. `analytics-web-app/src/routes/IngestionApiKeysPage.tsx`: single outer `<AuthGuard>`, calling `usePageTitle('Ingestion API Keys')` once ahead of the role branch, then branching admin (`<ApiKeysAdminPage>`, unchanged) vs. non-admin (new `IngestionKeysSelfServicePanel`, using the extracted hook/dialog). The panel calls the hook's `reload()` in a mount effect (mirroring `AudienceAccessPage.tsx`'s mount effect), and calls `reload()` from `onMinted` when `response.claimed` is true (mirroring `AudienceAccessPage.tsx`'s `handleMinted`). It also renders an `ErrorBanner` (retry calling `reload()`) for the hook's `error` field, mirroring `AudienceAccessPage.tsx`'s `myAudiencesError` handling. Its Mint button and the dialog's `open` prop are both gated on `me !== null`, mirroring `AudienceAccessPage.tsx`'s `showMintButton`/`open={mintOpen && me !== null}`.
...
8. Tests:
   - `AppShell.test.tsx`: add cases asserting the sidebar renders for a non-admin on `/admin` and `/admin/ingestion-keys`, and still doesn't on `/admin/maps` (existing case, unchanged). The `/admin/ingestion-keys` case needs a `vi.mock('@/routes/IngestionApiKeysPage', () => ({ default: () => <div>ingestion-api-keys-page</div> }))` added alongside the file's existing route mocks (`AdminPage`, `DataSourcesPage`, `ExportScreensPage`, `ImportScreensPage`, `MapsPage`, `NotFoundPage`), so the case exercises only the sidebar gate instead of mounting the real `ApiKeysAdminPage`/self-service-panel tree (whose mount effect calls `listIngestionApiKeys`/`fetchMyAudiences` against `fetch`, which this test file never stubs).
...
9. Docs: add a new "Admin hub" section to `mkdocs/docs/admin/web-app.md` describing the role-filtered card grid, and update `mkdocs/docs/admin/api-keys.md`'s "Web app admin pages" section, so both describe `/admin` and `/admin/ingestion-keys` as viewable-by-all with role-filtered content (the two now-viewable-by-all pages), distinct from the still-fully-admin-gated pages.
...
`mkdocs/docs/admin/web-app.md` (new "Admin hub" section) and `mkdocs/docs/admin/api-keys.md` ("Web app admin pages" section).
