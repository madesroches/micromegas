...
**No in-repo binary is currently affected.** All four in-repo call sites (across three binaries)
turn `None` into a hard abort — `rust/public/src/servers/flight_sql_server.rs:316-330`
(`use_default_auth`), `rust/monolith/src/main.rs:209-226` and `:232-250`,
`rust/telemetry-ingestion-srv/src/main.rs:60-72` — so any process that is actually running had
`configured == true` and does see a
...
///
/// Takes `&self` rather than `self` so `build()` can still reach the attached key
...
1. `rust/auth/src/default_provider.rs`: extract `compose(&self)` from the top of `build()`,
   returning `(MultiAuthProvider, bool)`; leave the existence query and the `!configured` return
   in `build()`.
2. Same file: add `pub async fn build_chain(self) -> Result<Arc<dyn AuthProvider>>` with the
   empty-chain `warn!`.
...
1. `build_chain_authenticates_key_minted_into_empty_table` (`#[ignore]`, `#[serial]`, live PG) —
   ...
   throwaway table needs the v5 `ingestion_api_keys` shape (`rust/ingestion/src/sql_migration.rs:128-137`)
   — `key_id`, `key_hash`, `name`, `created_at`, `created_by`, `last_used_at`, `revoked_at` — plus
   the later-added `audience` column, not just `key_id`/`revoked_at`.
2. `build_chain_issues_no_startup_query` (`#[serial]`, no DB, not `#[ignore]`) — `build_chain()`
   never calls `key_store_has_live_rows`, so composing it issues no query. Copy the short
   `PgPoolOptions::new().acquire_timeout(50ms).connect_lazy(..)` unreachable-pool helper into
   `default_provider_tests.rs` (matching `db_audience_grants_tests.rs:21`'s existing duplicate of
   `db_api_key_tests.rs`'s `unreachable_pool()`), same precedent as
   `dedicated_key_store_pool_is_small_and_lazy`: assert `build_chain()` returns `Ok` promptly
   against that unreachable pool, where `build()` returns `Err` (its existence query fails to
   connect). Use the file's `EnvGuard` and `remove_var` both `MICROMEGAS_API_KEYS` and
   `MICROMEGAS_OIDC_CONFIG` at the top, like every other test in this file — so an ambient
   `MICROMEGAS_API_KEYS` in the environment can't flip the `build()` assertion.
