...
**Time Range Behavior**: The query time range is used to *identify* which `block_id`s have duplicates, but once identified, ALL duplicate rows for that `block_id` are deleted regardless of their `insert_time`. This ensures complete cleanup of duplicates even if some copies exist outside the query range.
...
```sql
-- The CTE identifies block_ids with duplicates within the time range.
-- The DELETE then removes ALL duplicate rows for those block_ids,
-- regardless of insert_time, keeping only the earliest copy.
WITH dups AS (
...
| `rust/analytics/src/time.rs` | Modify (add `PartialEq, Eq, Hash` derives to `TimeRange`) |
