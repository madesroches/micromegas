...
`PageLayout.tsx` drops the `Sidebar` render (moved to `AppShell`) and pads its content row with `sm:pl-14` to leave room for the now out-of-flow sidebar rail. It keeps its own `h-screen bg-app-bg` wrapper and `Header` exactly as today, so `Header` still spans the full window width above the content row — the visual layout is unchanged from what's shipped today apart from the 1px header-height normalization in Step 2:
...
1. **Create `analytics-web-app/src/components/layout/AppShell.tsx`** — renders `Sidebar` once, gated on `useAuth()` status (plus an `/admin`-prefix check standing in for `requireAdmin`) so it doesn't mount during loading/unauthenticated/error/admin-denied states, wrapped in a `fixed top-16 left-0 bottom-0 z-40 hidden sm:flex` container so it renders out of flow instead of as a layout sibling of the page content, and wraps `<Outlet/>` in its own `<Suspense>` as a scoped loading fallback for the outlet region (not required to prevent a shell unmount — see Design) — per the snippet above. The wrapper's `z-40` is load-bearing, not cosmetic: `position: fixed` always creates a new stacking context, so without an explicit `z-index` the wrapper (and the flyout's `z-40` inside it, `Sidebar.tsx:329`) would paint in the root stacking context's "positioned, z-index: auto" layer, which loses to any positively-z-indexed page content — e.g. `ScreensPage.tsx:243`'s `z-10` card menu, `XYChart.tsx:1131`'s `z-10` header, `SwimlaneCell.tsx:333`'s `z-20` overlay, or `DateTimePicker.tsx:93`'s `z-30` backdrop — since the flyout's on-screen position overlaps the content column. Giving the wrapper `z-40` keeps the rail/flyout at the same stacking position relative to page content as today, where the flyout lives inside `<aside className="relative …">` (`Sidebar.tsx:284`) — `position: relative` with `z-index: auto` does not create a stacking context, so today the flyout's `z-40` already competes directly in the root stacking context against that same page content and wins.
...
- Manual verification in the browser (`yarn dev`):
  ...
  5. Confirm the page layout is unchanged from before this change (apart from the 1px header-height normalization in Step 2): on any page, `Header` still spans the full window width above the content row, and `Sidebar` still sits in its usual place below `Header` rather than as a full-height rail — including on a page with visible header controls (e.g. `/processes`), where hovering the sidebar open should show the flyout rendering below `Header` without covering the logo, time-range picker, or pivot button.
