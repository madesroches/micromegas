...
- No change to `computeFlexWidths()`: non-last column widths are still measured from the longest formatted string on the page (capped at 700px / 200px for `target`). When wrapping is on, a value longer than a non-last column's width simply wraps into multiple lines instead of being clipped — the existing cap just becomes a "reading width" rather than a hard clip point. This is a deliberate simplification: measuring by longest *individual line* (splitting on `\n`) instead of total string length would size columns more precisely for multi-line stack traces, but adds complexity for a secondary refinement; the plain approach is a reasonable starting point and can be revisited if the capped-width column reads too narrow in practice.
- **Last column bounds to available width, not a hardcoded cap.** The last column (typically `msg`) has no divider/column after it, so instead of a fixed measured width it uses `{ flexGrow: 1, flexShrink: 1, flexBasis: 0, minWidth: MIN_FLEX_WIDTH_PX }`: it grows to fill whatever row width is left over and shrinks down to `MIN_FLEX_WIDTH_PX` when space is tight. Its measured width (and thus the 700px cap) is irrelevant to its layout. Only when that minimum plus the fixed columns exceed the container does the row scroll horizontally — the ceiling is the available width, never a hardcoded constant. With wrap on it wraps within the available width; with wrap off it truncates at the available width.
...
**Width measurement unchanged (non-last columns)**: not re-deriving `computeFlexWidths` to measure per-line-length instead of total string length is a known simplification (see Design section) — a very long single-line-equivalent (e.g. one giant JSON blob with no newlines) in a *non-last* column will still size that column to the 700px cap and then wrap within that cap, which is the same as today's behavior minus the clipping. The last column is exempt: it bounds to the available row width (see Design), so a giant blob in `msg` wraps/truncates at whatever width is left rather than at 700px.
