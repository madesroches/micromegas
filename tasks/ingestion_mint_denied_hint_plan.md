...
- **Point a non-admin at `/audiences` from an Access-Denied screen instead of making `/admin`/`/admin/ingestion-keys` viewable** (the plan's original approach, superseded) — rejected on user direction: the admin screen should be viewable by all with role-filtered content, not gate-then-redirect. This also matches the issue's own suggested fix more directly.
...
4. `analytics-web-app/src/routes/IngestionApiKeysPage.tsx`: single outer `<AuthGuard>`, branch admin (`<ApiKeysAdminPage>`, unchanged) vs. non-admin (new `IngestionKeysSelfServicePanel`, using the extracted hook/dialog).
