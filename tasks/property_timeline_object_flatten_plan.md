...
- **Accepted migration gap — stale saved selections**: a `selectedProperties`/`selectedKeys` value persisted before this fix (in the URL query string or a notebook cell's `options.selectedKeys`) can reference a pre-flatten object-valued key name (e.g. `Dimensions`). After this fix ships, that raw key no longer exists in the parsed map — only its `Dimensions.<leaf>` descendants do — so `getPropertyTimeline('Dimensions')` finds nothing at any timestamp and the property row renders with no segments and no explanation, instead of the old `[object Object]`. This is accepted as out of scope, the same as the key-collision risk above: no migration or fallback code is added to detect/remap a stale object-valued selection.
