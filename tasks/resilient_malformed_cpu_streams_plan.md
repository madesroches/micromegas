...
- Confirm no row is inserted into `lakehouse_partitions` for a stream whose `jit_update` returns `Err`. This is an `#[ignore]` integration test gated on `MICROMEGAS_SQL_CONNECTION_STRING` (same pattern as the existing tests in `sql_view_test.rs`). Rather than querying `lakehouse_partitions` directly, assert the no-insert behaviour by calling `is_jit_partition_up_to_date` for the affected block IDs after the failed `jit_update` and asserting it returns `false` — a `true` result would mean a phantom partition was written.
