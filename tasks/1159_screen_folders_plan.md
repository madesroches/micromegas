...
- `GET /folders` → `Vec<FolderInfo>` (`path`, screen count, subfolder count) — union of explicit `folders` rows and implicit prefixes from `screens.folder_path`. No recursive CTE or string-aggregation SQL: consistent with the crate's existing flat-query style (and the Trade-offs section's own low-scale justification), fetch `folders.path` (via a plain `SELECT path FROM folders`) and distinct `screens.folder_path` (via `SELECT DISTINCT folder_path FROM screens`) with two flat queries, then in Rust split each path on `/` to expand ancestor prefixes into a `HashMap<String, FolderInfo>` and increment direct screen/subfolder counts as each row is folded in — same defaulting/aggregation-in-Rust approach already used elsewhere in this crate rather than in SQL. `HashMap` iteration order is unspecified, so before returning, collect into a `Vec<FolderInfo>` and sort it (e.g. by `path`) — matching `list_screens`'s explicit `ORDER BY name` — so the response order is deterministic instead of visibly unstable in the sidebar tree.
