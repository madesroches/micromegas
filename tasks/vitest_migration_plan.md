...
- `lucide-react` — 6 files (the sixth is `src/components/ui/__tests__/DateTimePicker.test.tsx`), each enumerating a fixed icon list; several carry confirmed value-position exposure, not just breadth risk:
  ...
  - The same `NotebookRenderer.test.tsx:23-44` factory also omits `AlertTriangle`, needed at value position by `src/components/ErrorBanner.tsx:1` (used `:29`), in-graph via `screen-renderers/shared.tsx:3`. Does not fire, for two reasons: `ErrorBanner.tsx:29` is `const Icon = isWarning ? AlertTriangle : AlertCircle`, and no `ErrorBanner` call site anywhere in the app — not `RendererLayout` (`shared.tsx:55-79`, rendering it at `:68`) nor any of its ~14 further render sites across 9 route components — ever passes `variant`, so `AlertTriangle` is never evaluated; and no test file renders any of `RendererLayout`'s four consumers (`MetricsRenderer.tsx:212`, `ProcessListRenderer.tsx:407`, `LogRenderer.tsx:610`, `TableRenderer.tsx:412`) at all.
...
- `process.env.NODE_ENV = 'development'` (line 39) can be dropped: nothing in `src/` reads `NODE_ENV`, Vitest runs in mode `test` so `import.meta.env.DEV` is `true`, and React resolves to its development build because `process.env.NODE_ENV !== 'production'` at runtime. One-line revert if anything regresses. `src/lib/config.ts:32` does read `import.meta.env.DEV`, but it's inert under test because `@/lib/config` is globally mocked at `test-setup.ts:42-45` and no test imports the real module. Separately, `vite.config.ts:40-42`'s single `define` entry — `'import.meta.env.VITE_BASE_PATH': JSON.stringify(basePath)`, where `basePath` comes only from `env.MICROMEGAS_BASE_PATH` (`vite.config.ts:8`) — now also reaches the test transform, and Vitest converts every `import.meta.env.*` define key into an assignment on `process.env` (`deleteDefineConfig` in `vitest@4.1.10`), so with `MICROMEGAS_BASE_PATH` unset the `JSON.stringify(undefined)` value lands as the string `"undefined"` on `process.env.VITE_BASE_PATH`. Harmless for the same mocking reason, and no `.env*` file exists in the repo.
...
11. Convert the 12 `vi.hoisted()` sites (Design §5) — `src/test-setup.ts`'s row was already handled by step 7, leaving 11.
12. Convert the 6 `importActual` / `importMock` sites (Design §7) — `importOriginal` inside factories, a direct `await import(...)` at `arrow-utils.test.ts:114` — `src/test-setup.ts`'s row was already handled by step 7, leaving 5.
