...
Only Prong A / Prong B disagreement: Prong B reads Postgres live, Prong A reads a
daemon-materialized snapshot of `processes`. In the window where the two disagree, the semi-join
would catch a row Prong B let through. Both prongs resolve the same row's own stamp, so the skew is
purely materialization-lag/retention-lag, and a materialized-but-not-yet-visible process is
invisible to Prong A in the *deny* direction, not the allow direction. This is not treated as a
realistic exposure and is not a reason to keep the predicate.
...
5. `rust/analytics/tests/ownership_rewrite_public_view_set_tests.rs`:
   ...
   - Update the `default_view_factory` inventory test (`~line 520`) from a two-way split to a
     three-way one: global view with an `audience` column → bare `Filter` on `audience`; global
     view without one → `LeftSemi Join`; `view_instance(...)`-reached view set → no injected
     predicate at all. The panic message must keep pointing at "a view set is missing a branch in
     `OwnershipRewrite`" for the planning failure case, since `classify`'s `Err` is unchanged.
...
6. `rust/analytics/tests/ownership_rewrite_db_test.rs`: the existing cross-audience sections for
   `net_spans`/`async_events`/`thread_spans` assert Prong B's uniform denial text and the owning
   caller's row counts; both must be unchanged by this work. Add an explicit assertion that the
   owning caller's row count for the three §4 view sets is identical before and after (in practice:
   that it is `> 0` and matches `ReadScope::All`), so a regression that over- or under-filters is
   caught.
...
9. `CHANGELOG.md`, Unreleased: one entry under the analytics/query section describing the dropped
   predicate, the view sets affected, and that it is a planning cleanup with no change to which
   rows any caller can read (#1530).
...
- `rust/analytics/tests/ownership_rewrite_db_test.rs` — row-count assertions
...
- A caller authorized for instance *X* saw exactly *X*'s rows before (the semi-join was a
  tautology over a partition set already confined to *X*) and sees exactly *X*'s rows after.
