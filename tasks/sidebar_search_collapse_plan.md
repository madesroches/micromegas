...
6. **Edit `analytics-web-app/src/routes/ImportScreensPage.tsx`** — call `notifyFoldersChanged()` after a successful `importScreen()` call (`ImportScreensPage.tsx:14`), matching every other mutation path (`ScreensPage.tsx:122,149`, `SaveScreenDialog.tsx:106`, `useMoveScreen.ts:18`). This gap already exists today but is masked because `Sidebar` currently remounts and refetches on every navigation; once `Sidebar` persists across navigation (this plan), an import would otherwise leave the sidebar's screen/folder tree stale until a full page reload.
