...
* **Web App:**
  * Add `analytics-web-srv` admin routes for the new audience grant store (#1489): `POST`/`GET`/`DELETE {base_path}/api/audience-grants`, `AdminUser`-gated like the existing key-management routes, structurally unreachable (not just gated) under `--disable-auth` via the same `key_management_disabled_router` mechanism. `POST` upserts via a single `INSERT ... ON CONFLICT DO NOTHING` CTE unioned with a fallback `SELECT` (201 on a fresh create, 200 reporting the pre-existing row, with one automatic retry on the rare zero-row race between two concurrent creators); `DELETE` takes the natural `(audience, axis, selector)` key as query parameters, not path segments, since a `group:<id>` selector's charset isn't restricted enough to be a safe raw path segment. See `mkdocs/docs/admin/authentication.md` and `mkdocs/docs/admin/api-keys.md`. **Minor breaking change**: `analytics_web_srv::web_server::build_protected_routes` (published) replaces its separate `AnalyticsKeysState`/`IngestionKeysState` parameters with a single new `AdminRoutesState` struct that bundles those two plus the new `AudienceGrantsState`.
* **Python:**
  * Add the `micromegas-grants` CLI (`create`/`list`/`delete` subcommands) and three matching `WebClient` methods (`create_audience_grant`/`list_audience_grants`/`delete_audience_grant`), talking to the new `/api/audience-grants` routes over HTTP — never direct Postgres access, the same convention every CLI in this codebase follows (#1489).
* **Services:**
