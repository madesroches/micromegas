...
* **Build:**
  * Bump `react-router-dom` to `react-router@^8.3.0` in `analytics-web-app` (the package was renamed starting with v8), and bump `.nvmrc` from Node 20 to `22.22.0` to satisfy `react-router@8.3.0`'s `engines.node >=22.22.0` floor, to resolve Dependabot alert #388 (GHSA-qwww-vcr4-c8h2)
