...
* **Build:**
  ...
  * Bump `brace-expansion` to `^5.0.8` via yarn `resolutions` across the root, `welcome/`, and `analytics-web-app/` workspaces (the older 1.x-4.x lines never received a patched release, only 5.x did) to resolve Dependabot alerts 396-398 (GHSA-mh99-v99m-4gvg); Dependabot alert 395 (`react-router` CSRF bypass, GHSA-qwww-vcr4-c8h2, root workspace) stays open — react-router 8.x dropped CommonJS support entirely, which breaks the legacy `react-router-dom-v5-compat` shim that `@grafana/ui` requires eagerly at module load, so it can't be bumped past 7.x without a separate `@grafana/ui` major upgrade
