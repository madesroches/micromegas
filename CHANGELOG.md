...
* **Caching:**
  ...
  * Add `MICROMEGAS_DISABLE_METADATA_PSQL_CACHE` (default off): when set, `ParquetReader` reads partition Parquet footers directly from object storage through the object-cache-backed reader instead of the postgres `partition_metadata` table, keeping the in-process parsed `MetadataCache` in front of both paths so only the miss-backfill source differs — a read-only A/B toggle to compare the two before deciding whether to retire the postgres path (#1231)
* **OTLP:**
