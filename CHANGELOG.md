...
* **Ingestion:**
  ...
  * **Known gap, documentation-only for now**: the conflict guard's `NULL`→no-op branch (an existing row with no stamped audience is left alone, so a mid-migration re-registration doesn't lose its process) has its own confidentiality gap, distinct from the squatting gap above: a credential with no bound audience (env-keyring key, OIDC, or `--disable-auth`) can pre-register a victim's future `process_id` unstamped via `insert_process`, and the victim's genuine producer's later registration then hits the same `NULL`→no-op branch and silently no-ops instead of stamping the row — permanently suppressing the victim's stamp. Since `MICROMEGAS_UNSTAMPED_AUDIENCE` defaults to `public` (see the **Analytics** entry below), this makes the victim's data world-readable out of the box, not just under an opt-in migration setting. There is no in-product enforcement knob that closes this; the mitigation is operational — provision only audience-bound DB-backed ingestion credentials, and don't run ingestion with `MICROMEGAS_API_KEYS`, ingestion OIDC, or `--disable-auth` alongside them, or set `MICROMEGAS_UNSTAMPED_AUDIENCE` to an empty string to opt back into fail-closed — tracked as a follow-up stage of the AbAC epic (#1334) / to #1373 (Stage 5b).
