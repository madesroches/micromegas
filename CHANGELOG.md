...
* **Analytics:**
  * Merges (`QueryMerger`, the path every non-ordering-declaring view uses -- `measures`,
    ...
    task could absorb. `execute_merge_query`'s three-way dispatch also collapses to the two
    strategies that actually exist -- concatenate (one sequential reader) and sort-merge (k
    readers collapsed by a `SortPreservingMergeExec`) -- since `Unordered` and `Concatenated`
    build and execute the identical scan and only differ in whether the resulting order is
    declared. For a non-aggregating merge query (the default `SELECT * FROM source` shape most
    views use), this makes merged-partition row order deterministic -- source-partition
    concatenation order, in place of the previous nondeterministic byte-range interleave -- and
    restores row-group pruning for time-filtered queries against the resulting time-local row
    groups; an aggregating (`GROUP BY`) merge query's output order stays as nondeterministic as
    before. No sort guarantee is recorded or claimed for any view by this change -- it is ordering
    by construction, not a contract. `create_merged_partition` also logs a per-merge elapsed-time
    completion line alongside the existing `sum_size` line, so before/after wall-clock is
    queryable from `log_entries` without new instrumentation.
  * Add `OwnershipRewrite`, Query Enforcement Prong A of the AbAC rollout (#1370, Stage 2 of the epic tracked at #1334) — a mandatory `AnalyzerRule` that injects an audience-filtering predicate into every `MaterializedView`-backed query plan, consuming the `ReadScope` Stage 1 (#1369) already threads into `make_session_context` but did not yet enforce. Resolves one audience per process (`Aggregate(GROUP BY process_id, MAX(audience))` over the raw, time-unbounded `processes` partitions, `NULL`s ignored so a stamped row always outranks an unstamped one) rather than filtering raw partition rows directly, closing a fail-open gap the parent plan's own `IN (SELECT process_id FROM processes WHERE ...)` construction would have left (a process's pre-stamping rows would otherwise keep it visible under the escape hatch below forever after a later, narrower stamp). Branches per view set: `processes`' own scan and every process_id-**column** view (`streams`, `blocks`, `log_entries`, `measures`, `net_spans`, `otel_spans`, `images`, `log_stats`) get a `process_id IN (subquery)` semi-join; `async_events` and `thread_spans` — process/stream-scoped but with no `process_id` (or, for `thread_spans`, no `stream_id`) column to join on — get a literal-valued `EXISTS` keyed on `get_view_instance_id()` instead (`thread_spans`' additionally resolves through `streams`); a new `MICROMEGAS_PUBLIC_VIEW_SETS` allowlist (comma-separated, empty/off by default) skips the predicate entirely for named, genuinely aggregated/non-PII view sets; any view set matching none of the above makes `analyze()` fail loudly (`DataFusionError::Plan`) rather than silently plan an unfiltered scan. `ReadScope::All` (the internal/maintenance marker) is a true no-op. A new `MICROMEGAS_UNSTAMPED_AUDIENCE` knob (`{prefix}_UNSTAMPED_AUDIENCE`, falling back to the unprefixed form) names the audience a process with no `micromegas.audience` property coalesces to — **defaults to `public`** when genuinely unset; an operator sets it to an empty string explicitly to opt into the fail-closed alternative (unstamped processes invisible to every `ReadScope::Audiences` caller). Both knobs are bundled into a new `IsolationConfig`, parsed in `micromegas-analytics` (not `micromegas-auth`, mirroring Stage 1's own crate-boundary reasoning) and carried on a new `CallerContext.isolation_config` field rather than a new `make_session_context` parameter (renamed from `OwnershipRewriteConfig`/`ownership_config` once Stage 3, #1371 below, added a third knob this config object carries that has nothing to do with the ownership rewrite). **Upgrade note, read before deploying with auth enabled**: any deployment already running with an `AudienceReadPolicy` active goes from full visibility today (nothing yet filters a `ReadScope::Audiences` session) to filtered-by-audience the instant this ships. `MICROMEGAS_UNSTAMPED_AUDIENCE` defaulting to `public` keeps pre-existing, unstamped data visible with no operator action required — `public` is the sole built-in read grant every authenticated principal has under #1372's grant-map model below, so this one default is enough (that model replaces the identity-derived singleton `{user:<email>}` an earlier revision of this note had to widen with a second `MICROMEGAS_IMPLICIT_GROUPS` knob; #1372 removes that knob outright, and the relaxed audience charset it ships makes the originally-recommended `MICROMEGAS_UNSTAMPED_AUDIENCE=group:everyone` fail startup, so use `public` instead). A deployment that wants the fail-closed alternative (**zero visible rows** for unstamped data) sets `MICROMEGAS_UNSTAMPED_AUDIENCE` to an empty string explicitly. Ingestion-time stamping has since landed (Stage 5, #1373), so this knob now matters only for data ingested *before* that stage shipped, or ingested under a credential with no bound audience (env-keyring key, OIDC) — see that entry below for what does and doesn't carry a write audience. An auth-unset deployment (no `AuthContext` extension, e.g. `--disable-auth`) is completely unaffected. **This escape hatch has a further precondition the knob alone doesn't satisfy**: `OwnershipRewrite` resolves a process's audience from the *materialized* `processes`/`streams` views (an `Aggregate` over `__processes__partitions`), and those are `SqlBatchView`s whose `jit_update` is a no-op — they are only ever populated by the maintenance daemon's periodic `materialize_all_views` pass. A process entirely absent from that materialized view contributes nothing to the coalesced `IN`/`EXISTS` check regardless of the env var, so a running, caught-up maintenance role is an additional, undeclared precondition: every process ingested since the daemon's last pass, and *all* processes if the daemon isn't running or deployed, stays invisible to every `ReadScope::Audiences` caller — including a caller's own just-ingested data — until the daemon catches up. **Known limitation, not yet closed**: `micromegas.audience` is read as a plain, client-supplied process property with no server-side validation — any instrumented process can set it to whatever value it likes, including one that hides its own data from every legitimate caller or spoofs another principal's audience. The intended trust anchor is the ingestion API key, not the client payload: each key is assigned exactly one write audience (Stage 4, #1372 below) carried authenticated into `AuthContext.bound_audience`. Stage 5 (#1373, see the **Ingestion** entry below) has since landed and closes this gap — `micromegas.audience` is now stamped server-side from that authenticated value, and a client-supplied `micromegas.*` property is stripped rather than trusted — so this stage's enforcement is now a real security boundary against a malicious or misconfigured instrumented client, not merely a client-asserted label. Nor was it one against an authenticated *reader*: `process_spans`, `perfetto_trace_chunks`, and `parse_block` each planned their own internal session under `ReadScope::All`, which made `OwnershipRewrite` a no-op for them — a restricted, authenticated caller could read any process's span/block data through these three table-valued functions, completely unfiltered by audience. This gap is closed by Query Enforcement Prong B (#1371, Stage 3, above), which adds arg-addressed audience guards to these surfaces (plus `get_payload` and `list_partitions`). **Minor breaking change**: `FlightSqlServiceImpl::new` (published API, `micromegas::servers::flight_sql_service_impl`) gains a required `isolation_config: Arc<IsolationConfig>` parameter; the public, struct-literal-constructed `CallerContext` (`micromegas_analytics::lakehouse::read_scope`) gains a new public field, `isolation_config`. `FlightSqlServerBuilder` gains `with_isolation_config()`. **Also breaking for a caller-supplied `ViewFactory`**: `make_session_context` now requires the `ViewFactory` (`FlightSqlServerBuilder::with_view_factory_fn`, published API) to register both the `processes` and `streams` global views whenever a request's `ReadScope` is not `ReadScope::All` — `OwnershipRewrite` reads its audience mapping from them — so a custom `ViewFactory` that omits either one now fails every such request at session creation instead of succeeding. **Amended (#1482, still `## Unreleased`)**: `audience` is now a physical, non-nullable column materialized directly on `processes`, `streams`, `blocks`, `log_entries`, `measures`, and `log_stats` (see the new **Analytics** entry below), so those six view sets are filtered with a direct `audience IN (...)` predicate on their own column — no semi-join, no `property_get`, no per-process aggregate. The `process_id IN (subquery)` semi-join described above now applies only to `net_spans`, `otel_spans`, and `images`, which don't carry the column yet; `async_events` and `thread_spans` keep their literal-valued `EXISTS` shapes unchanged. The `MICROMEGAS_UNSTAMPED_AUDIENCE` knob this entry originally introduced is removed outright, along with the "unstamped" state and every escape-hatch caveat above about it — every process now carries an audience unconditionally (see the **Ingestion** entry below), so there is nothing left to coalesce at query time. A deployment built from `main` with `MICROMEGAS_UNSTAMPED_AUDIENCE` (or `{prefix}_UNSTAMPED_AUDIENCE`) still set now fails at startup, naming `MICROMEGAS_DEFAULT_AUDIENCE` as its replacement.
  * Add `AudienceGuard`, Query Enforcement Prong B of the AbAC rollout (#1371, Stage 3 of the epic tracked at #1334) — arg-addressed audience guards for the four span/metadata surfaces `OwnershipRewrite` (Prong A, above) structurally cannot reach (`process_spans`, `perfetto_trace_chunks`, `parse_block`, and the `get_payload` UDF: each bakes its target id into a provider at plan time and returns a schema with no `process_id` column to filter on, and three of them planned their own inner session under `ReadScope::All`, a no-op for `OwnershipRewrite`), plus row filtering on `list_partitions`. All fed by a new `AudienceIndex`, a size- (100k entries, ~10 MB) and TTL-bounded (5 minutes) `moka` cache resolving *any telemetry id → its owning process's audience* by indexed point query against Postgres (the origin of `micromegas.audience`, fresher and independent of the maintenance daemon's materialization that Prong A depends on) — keyed on `(IdKind, Uuid)`, not the bare id, since the same client-supplied UUID can legitimately be a `process_id` in one audience and a `stream_id`/`block_id` in another. `process_spans`/`perfetto_trace_chunks`/`parse_block` now parse their id argument as a `Uuid` at plan time (closing an incidental SQL-injection vector into their inner session, since that string was previously interpolated verbatim) and run the async `AudienceGuard::authorize` check inside `TableProvider::scan`, before any data is touched; a resolution error is a denial, never a pass. Guarded, the three then run their *inner* session under a witness type (`Authorized`, constructible only by a successful `authorize` call) whose `internal_caller()` still resolves `ReadScope::All` — a deliberate deviation from naive scope inheritance, since every inner statement they build is server-constructed and confined to the id the guard already authorized, so inheriting the caller's scope would only add a second, redundant daemon-freshness dependency with no security benefit. `get_payload` resolves its distinct `process_id` arguments in one batched round trip and denies the whole call if any is unreadable, never a per-row `NULL` (which would be indistinguishable from a missing payload). `list_partitions` drops its `LIMIT` pushdown for a `ReadScope::Audiences` caller (filtering after a pushed-down limit could silently under-return), row-filters by resolving each `view_instance_id` that parses as a UUID, and hides `'global'` rows (no single audience to check) unless `MICROMEGAS_UNSTAMPED_AUDIENCE` is in the caller's scope, the row's view set is in `MICROMEGAS_PUBLIC_VIEW_SETS`, or the caller holds `ReadScope::All`. **Amended (#1482, still `## Unreleased`)**: `OwnerAudience::Unstamped` is removed along with the query-time unstamped state it modeled (a resolved `None` audience now maps to `OwnerAudience::Unknown`, always denied under `ReadScope::Audiences` — an invariant violation once every process is stamped at write time, not a state to special-case); `AudienceGuard::new` drops its `unstamped_audience` parameter in favor of a `lakehouse_admin: bool`, occupying the same slot. `list_partitions`' `'global'`-row rule changes to match: a row is visible when its view set is in `MICROMEGAS_PUBLIC_VIEW_SETS` **or** the caller passes the lakehouse admin gate (`caller.is_admin || !caller.admin_principal_possible`, the same boolean already gating the five mutating UDTFs/UDFs) — not merely because the caller's scope happens to include whatever the removed knob was configured to. Every denial and every "no such id" produce the same not-found-shaped error text — a distinct "permission denied" would be an existence oracle letting a caller enumerate ids in other audiences; the server log (`debug!`) still records the real reason. `ReadScope::All` (internal/maintenance callers, and any auth-unset deployment) is a true no-op throughout, with no code path touching Postgres. **Also lands the parent plan's deferred mutating-function registration gate, derived rather than knob-configured**: the five mutating lakehouse UDTFs/UDFs are registered for *every* authenticated caller instead of admin-only whenever this deployment can never produce an admin principal at all, restoring access for an API-key-only deployment that #1382's `is_admin`-only gate (rightly) left with none, since an API key can never be admin. Whether a deployment can ever produce an admin is now a capability the auth layer answers at startup, not an operator-set env var: a new `AuthProvider::can_grant_admin()` method (defaulted `false`) is overridden by `OidcAuthProvider` to `true` only when its `admin_users` list is non-empty (an OIDC deployment with no configured admins has no admin principal either), and by `MultiAuthProvider` to `true` if any provider in its chain is; both API-key providers keep the default. `flight_sql_server.rs` computes this once per server across all three auth-resolution branches (including the monolith's injected-provider path) and carries it on `FlightSqlServiceImpl`, which copies it onto every `CallerContext` it resolves as a new `admin_principal_possible: bool` field; `query.rs`'s gate becomes `caller.is_admin || !caller.admin_principal_possible`. **Deployment-wide, not per-audience**: none of the five functions filters by audience, so on a deployment with no admin principal, every authenticated caller gets destructive access to every audience's partitions — safe only when the deployment has no admin principal to begin with, unsafe the moment it also has personal or per-team audiences. **Minor breaking change**: `ListPartitionsTableFunction::new`, `GetPayload::new`, `PerfettoTraceTableFunction::new`, `ParseBlockTableFunction::new`, and `ProcessSpansTableFunction::new` (published, `micromegas_analytics::lakehouse::*`) each gain a required `Arc<AudienceGuard>` parameter. `PerfettoTraceExecutionPlan::new`/`PerfettoTraceTableProvider::new` are narrowed from `pub` to `pub(crate)`, matching `process_spans`' existing module-private shape (no external crate could otherwise build an un-authorized execution plan bypassing `scan`). `ListPartitionsTableProvider` (published, previously a single-field, externally struct-literal-constructible `pub` struct) gains a private `guard: Arc<AudienceGuard>` field, so it can no longer be built outside this crate by a struct literal. **Renames `OwnershipRewriteConfig` → `IsolationConfig`** (not a break against any released API — the type was introduced under this same still-`## Unreleased` Stage-2 entry above, so every mention there is updated in place rather than getting its own breaking-change clause): `CallerContext.ownership_config` → `isolation_config`, `FlightSqlServiceImpl::new`'s parameter, and `FlightSqlServerBuilder::with_ownership_config()` → `with_isolation_config()` — a clean name over a compatible one, per `CLAUDE.md`'s Rust-API-agility stance. `AuthProvider` (published, `micromegas_auth::types`) gains the `can_grant_admin()` method described above, defaulted so every existing implementor keeps compiling unchanged; `CallerContext` (published, struct-literal-constructed) gains the `admin_principal_possible` field above, so every downstream construction site needs updating; `FlightSqlServiceImpl::new` gains a required `admin_principal_possible: bool` parameter.
* **Auth:**
  * Give every `ingestion_api_keys` row a single, immutable write audience, carried authenticated into `AuthContext.bound_audience` (#1372, Stage 4 of the epic tracked at #1334) — the value Stage 5 (#1373, see the **Ingestion** entry below) stamps `micromegas.audience` from. **Also settles what an audience *is*, replacing Stage 1's (#1369) prefixed, identity-derived model**: an audience is now an opaque label (`is_valid_audience`: `[A-Za-z0-9_-]{1,255}`, case-sensitive, no normalization) on a bucket of data, and who may read/mint into it is separate, editable configuration — a new `AudienceGrants` map, parsed from JSON at `{prefix}_AUDIENCE_GRANTS`/`MICROMEGAS_AUDIENCE_GRANTS` and keyed by audience name, each value either a bare array (read-only shorthand) or `{"read": [...], "mint": [...]}` for an audience that also grants mint authority; selectors are `*` (anyone authenticated), `user:<email>`, or `group:<g>`, validated at parse time (unknown-shaped key/selector or a duplicate JSON key ⇒ startup `Err`, not a silently-inert entry). `public` is the sole built-in: every authenticated principal reads it, with no config needed. There is **no self-audience rule** (no "you may read the audience named after you") — the charset makes an email unrepresentable as an audience name, and keying on `subject` instead would let an admin mint themselves read access by naming a key after an audience; a personal audience is now an ordinary audience with an ordinary per-user grant, deferred to Stage 6 (#1374), which is what lets a user mint their own key in the first place. `AudienceReadPolicy::new`/`AudienceMintPolicy::new` now take an `AudienceGrants` instead of an implicit-groups list; `MintPolicy::resolve_audience(caller, None)` is now always an `Err` ("no audience requested and none can be defaulted") — there is no "myself" audience to default to under the opaque-label model. **Migration v6** adds `ingestion_api_keys.audience VARCHAR(255)`, backfilled to `'public'` (the accurate description of every pre-#1372 key's current, unstamped-and-visible-to-everyone state) before `SET NOT NULL`, plus a `CHECK (audience ~ '^[A-Za-z0-9_-]+$')` mirroring `is_valid_audience`. `DbApiKeyAuthProvider`'s loader reads the column back (table-conditional `RETURNING`, `ingestion_api_keys` only) into `bound_audience`; `allow_delegation` is now `false` for ingestion keys (`true`, unchanged, for analytics keys) — a write credential is not a delegating service account, though this is currently inert since an ingestion key can never reach the gRPC path that flag governs. `analytics-web-srv`'s mint/import routes for `ingestion_api_keys` gain an optional `audience` request field and a matching response field (`mint` requires an explicit audience or `MICROMEGAS_DEFAULT_AUDIENCE` — a **400** if neither resolves, never a silent `public`, since that would publish every future process the new credential ingests; `import` falls back to `public` when neither is set, matching the v6 backfill's continuity assumption for a pre-existing key). **Amended (#1482, still `## Unreleased`)**: that mint-side 400 is gone — `MICROMEGAS_DEFAULT_AUDIENCE` always resolves (`public` when unset), so a mint request naming no audience mints for the deployment default on both routes; see the collapsed-knob entry below. New Admin → Ingestion API Keys "Audience" table column and mint-dialog input in the web app (analytics keys are unaffected — they carry no audience). `micromegas-import-keys` gains `--audience` (ingestion only) and a per-key keyring `"audience"` field (wins over `--audience`; combined with `--table analytics` it's a startup error, checked up front rather than partway through a batch of live imports). **Operator-facing break, pre-GA**: `MICROMEGAS_IMPLICIT_GROUPS`/`{prefix}_IMPLICIT_GROUPS` (introduced in v0.29.0, #1369) is removed outright, subsumed by `{"<name>": ["*"]}` in the grant map; a previously-recommended `MICROMEGAS_UNSTAMPED_AUDIENCE=group:everyone` now fails startup under the relaxed charset (`:` is outside `[A-Za-z0-9_-]`). **Amended (#1482, still `## Unreleased`)**: `MICROMEGAS_UNSTAMPED_AUDIENCE` is removed outright and now fails startup if set at all, to any value — drop it entirely and rely on `MICROMEGAS_DEFAULT_AUDIENCE` (default `public`) instead, along with dropping `MICROMEGAS_IMPLICIT_GROUPS` entirely. **A `NOT NULL` column with no `DEFAULT` also imposes a deploy-order requirement**: roll `analytics-web-srv` to this change in the *same* deploy that runs the v6 migration (ingestion/monolith) — running the migration first without also rolling the web service, or the reverse, both produce an outage window (`NOT NULL` violation, or a missing-column 500, respectively) on the mint/import routes until both sides catch up — see `mkdocs/docs/admin/api-keys.md`. **Minor breaking change**: `micromegas_auth::policy`'s published surface changes shape — `AudienceReadPolicy::new`/`AudienceMintPolicy::new` now take an `AudienceGrants`, and `MintPolicy::resolve_audience`'s `None` contract is now always an error; `micromegas_auth::default_provider::implicit_groups_var` (published) is removed outright. `IngestionKeysState` (published, all-public-fields, `analytics-web-srv`) gains a required `default_audience` field.
* **Ingestion:**
  * Stamp `micromegas.audience` server-side from the authenticated ingestion credential instead of trusting whatever the client sent (#1373, Stage 5 of the epic tracked at #1334) — the fix `ownership_rewrite.rs`'s known Stage 2 limitation pointed at. Every `insert_process`/`register_otel_process` call now resolves a `WriteAudience` from `AuthContext.bound_audience` (`Some` for every DB-backed ingestion key, `None` for env-keyring keys, OIDC, and no-auth-provider deployments) and writes it as the reserved `micromegas.audience` property; any client-supplied property under the reserved `micromegas.*` namespace is dropped at ingestion (`warn!`-logged, naming the key) so the namespace can never be asserted from the payload. `WriteAudience::none()` writes no property at all — absent, not empty, matching what `OwnershipRewriteConfig.unstamped_audience` already coalesces. No retro-stamping: an existing process's audience never changes after the fact. One audience per process is now enforced on **both** the native `insert_process` path and the OTLP `register_otel_process` path: a re-registration of an existing `process_id` under a *different* audience than it was originally stamped with is now a 403 (`IngestionServiceError::AudienceConflict`); a re-registration under the same audience, or of a still-`NULL` (never-stamped) row, remains a no-op. This is what makes Stage 2's `MAX(audience)`-per-process resolution (`ownership_rewrite.rs`) sound rather than merely assumed. The `register_otel_process` guard closes a confidentiality gap, not just an integrity one: `processes` is a single table shared with the native path, and since the OTLP `process_id` derivation formula is public, any ingestion credential could pre-register (via the native path) the exact `process_id` a victim audience's OTLP producer would later derive — without this check, the genuine producer's stream/blocks would silently land on a row stamped with the squatter's audience, leaking that audience's data to the squatter. Whichever party registers second gets the 403: in this squatting scenario that's the victim's own later, genuine registration, not the squatter's, and since a stamped process's audience is immutable (no `UPDATE processes` path exists anywhere in the codebase), recovery requires an operator to manually delete the squatted row (e.g. `DELETE FROM processes WHERE process_id = ...`) — the maintenance daemon's `delete_empty_processes` sweep (`rust/analytics/src/delete.rs`) only reclaims it automatically once the squatted row has no streams and the retention window has elapsed. **Amended (#1482, still `## Unreleased`)**: every process now gets an audience, always — `WriteAudience::none()` is removed, and a credential with no bound audience is stamped with the deployment's `MICROMEGAS_DEFAULT_AUDIENCE` (default `public`) instead of going unstamped. An idempotent backfill runs at every ingestion-service startup, appending that same default onto any legacy `processes` row that predates this change and was never stamped at all — safe to re-run, and what repairs a straggler row an old replica wrote mid-rollout. The conflict guard's existing-`NULL`-audience "no retro-stamp" arm is gone with it: a row with no `micromegas.audience` property after the backfill is now an invariant violation, rejected as a database error rather than silently left alone.
  * Make OTLP-derived `process_id`/`block_id` audience-scoped (#1373, Stage 5, same epic), closing a structural gap stamping would otherwise expose: `process_id_from_resource` previously hashed only client-supplied resource attributes, so two audiences posting identical resources (the same containerized app in two tenants, a degenerate resource, a CloudWatch namespace) derived the same `process_id` — the first one there would own the row and its audience, silently mislabeling the second's data and, since `blocks` also dedups on content-addressed `block_id` alone, silently dropping byte-identical writes from the second audience entirely. A new `IdentityContext { audience, extra_hash_input }` threads the resolved write audience (and the webhook path's existing header-hash input) into both formulas: `process_id_from_resource` hashes the joined resource-attribute key under a **per-audience namespace UUID** (`NS_OTEL_PROCESS_V1` salted with the audience) when the audience is `Some`, rather than appending the audience string onto the same `\x1F`-joined key the 31 resource fields share — appending it there would have let a resource-attribute value crafted to end in `\x1F<audience>` reproduce another audience's stamped key byte-for-byte, since OTLP attribute values are arbitrary UTF-8 and nothing escapes `\x1F` in them. `block_id`'s hash input gains an `aud\x1F<audience>\x1F` prefix ahead of any `extra_hash_input`. Both formulas are no-ops (byte-identical ids) when the audience is absent, so an unstamped deployment sees zero churn. A deployment that *starts* stamping re-derives its OTLP `process_id`s: the same logical process appears as a new row, and its pre-upgrade data keeps the old id and stays unstamped. Rotating an ingestion key to a different audience likewise splits a long-lived producer's history across two process ids — expected, since the data now genuinely belongs to two audiences. **Amended (#1482, still `## Unreleased`)**: with `WriteAudience::none()` removed, `IdentityContext.audience` is now a plain `&str`, never absent — every previously-unstamped deployment (default-audience `public` under both the old read-side knob and the new write-side one) churns its OTLP ids exactly once on upgrade, the same shape this entry already documents for a deployment that newly starts keyed ingestion.
  * Firehose routes now propagate the `AuthContext` they already validate instead of discarding it: `firehose_auth_middleware` used to authenticate the delivery-stream's access key and then throw the result away (`Ok(_ctx) => { ... }`), so a bound audience on a Firehose credential never reached ingestion. It now inserts the context into the request extensions, exactly like the shared Bearer `auth_middleware` — Firehose and CloudWatch Logs deliveries are stamped the same as every other entry point.
  ...
  * **Minor breaking change**: `WebIngestionService::insert_process` and `register_otel_process` (published, `micromegas_ingestion::web_ingestion_service`) each gain a required `&WriteAudience` parameter; `micromegas_otel_ingestion::identity::process_id_from_resource` and `block::{split_logs, split_metrics, split_traces}` each gain a required `IdentityContext` parameter, and `block::split_logs_with_extra_hash_input` is removed outright, collapsed into `split_logs`; `handler::{ingest_logs, ingest_metrics, ingest_traces, ingest_webhook, ingest_firehose_metrics}` and `cloudwatch_logs::ingest_cloudwatch_logs_firehose` each gain a required `&WriteAudience` parameter; `resolve_write_audience` (published, `micromegas::servers::write_audience`) takes `Option<&Extension<AuthContext>>` and returns `WriteAudience` directly — infallible, no `Result` and no `StampingConfig`/enforcement-knob parameter, since the write-audience gate this stage originally shipped alongside stamping was removed before release (a bound-audience credential always stamps; an audience-less one resolves to `WriteAudience::none()` unchanged, no warning, as before; a malformed `bound_audience` now warns and resolves to `WriteAudience::none()` instead of being rejected — a deliberate fail-open narrowing of the defence-in-depth check this same entry originally described as fail-closed, not a behavior-preserving refactor). **Upgrade note**: expect a one-time OTLP `process_id` re-derivation the moment a deployment starts stamping (see above); a process that previously self-stamped `micromegas.audience` on its own while authenticating with an audience-less credential (env-keyring key, OIDC) silently becomes unstamped on upgrade instead of keeping its self-asserted label — move it onto a DB ingestion key bound to that audience to keep its own label. FlightSQL `do_put_statement_ingest` (`CommandStatementIngest`, `bulk_ingest`) now requires an admin credential (`is_admin(request.metadata())`, `Status::permission_denied` otherwise) — `replication.rs`'s `bulk_ingest`/`ingest_processes` write row properties, including `micromegas.audience` on `processes` rows, straight through with no server-side stamping or reserved-namespace stripping (unlike the HTTP ingestion paths above), so any authenticated FlightSQL client could otherwise set `micromegas.audience` directly and bypass this stage's stamping entirely; gating the RPC to admins closes that gap while preserving `bulk_ingest`'s documented purpose (`mkdocs/docs/query-guide/python-api.md`) of replicating a process's audience verbatim from its origin lake. Concretely, "admin credential" means an OIDC-authenticated identity listed in `MICROMEGAS_ADMINS` (or a `--disable-auth` deployment); API-key credentials (both `ingestion_api_keys` and `analytics_api_keys`) are hardcoded to `is_admin: false` and can never satisfy this gate, so any previously-working `bulk_ingest` automation authenticating with an API key now gets a permanent `PERMISSION_DENIED` and must switch to an OIDC-based admin identity. **Amended (#1482, still `## Unreleased`)**: `WriteAudience` is now `WriteAudience(Arc<str>)` — `none()` is removed, `as_str()` returns `&str` (not `Option<&str>`), and a new `WriteAudience::default_from_env()` resolves `MICROMEGAS_DEFAULT_AUDIENCE` (default `public`). `resolve_write_audience` now takes a `default: &WriteAudience` and returns `Result<WriteAudience, _>` — a malformed `bound_audience` is rejected outright rather than degraded to `none()`, since that degrade would now move a restricted key's writes into the default audience instead of leaving them unstamped. `WebIngestionService::new` and `micromegas::servers::ingestion::serve_ingestion` each gain a required `default_audience: WriteAudience` parameter (a `new_for_test` helper, defaulting to `public`, covers the ~25 test call sites that don't care); `micromegas_otel_ingestion::identity::IdentityContext.audience` is `&str` and the struct no longer derives `Default`.
* **Web App:**
  * Suppress the y-axis tick unit suffix on single-line charts when the unit has no known short abbreviation (e.g. `ops_per_sec`, `requests`): repeating a long, opaque unit string on every tick risked truncation, and the unit remains visible in the chart header, stats row, and tooltip regardless. Multi-line charts are unaffected. A new `isCompactAxisUnit` predicate in `units.ts` decides this from `CANONICAL_DISPLAY_ABBREV` membership plus a length-bounded check for short symbol-prefixed units (`/s`, `°F`, `%CPU`)
  * **Fixed**: y-axis tick labels no longer clip at the canvas edge — the axis band now widens to fit its formatted labels instead of staying a hard-coded 90px, never shrinking below that size so a chart that already rendered correctly is unaffected (#1503)
* **Unreal:**
...
* **Analytics:**
  ...
  * Fix `get_process_thread_list` (`process_streams.rs`, backing `process_spans` and `perfetto_trace_chunks`) panicking on a process whose streams have no `thread-name`/`thread-id` property: `property_get` returns `NULL` for an absent property, and a dictionary-encoded column whose every value in a batch is `NULL` has an empty values buffer, so reading it by key panicked instead of returning the string. Both columns now check `is_null` before `value()`, falling back to the same empty string the existing display-name logic already treats a present-but-empty property as. Surfaced by Prong B's (#1371) new guard tests exercising such a process; the bug predates and is unrelated to that stage.
  ...
  * Add an admin-managed query deny list (#1488): a Postgres-backed, replica-cached set of rules, checked at the front of `execute_query` -- before the session context is built and before planning -- so a matching query is rejected for microseconds of work instead of a memory-pool reservation and a wave of object-store reads. A rule is a boolean SQL expression (parsed and evaluated by DataFusion itself, so there's no matcher language of our own to specify) over a fixed match context -- `user_id`, `email`, `service_account`, `client`, `agent`, `entrypoint`, `session`, `notebook`, `cell`, `client_ip`, `sql`, and a new normalized `sql_hash` fingerprint (literal-stripped, so a dashboard re-issuing the same query on every refresh collapses to one fingerprint) -- validated with exactly two checks (result type is `Boolean`; at least one column is referenced, so a rule can't accidentally deny every query) and otherwise unconstrained: a top-level `OR` with no anchoring equality is deliberately legal, since a blanket `sql LIKE '%view%'` rule is one of the strongest incident levers an on-call admin has. Administered from SQL, admin-gated exactly like `retire_partitions` -- `list_query_denials()`, `deny_queries(match_expr, reason)`, `remove_query_denial(rule_id)` -- and from a new Admin -> Query Deny List screen in the analytics web app that drives those same three functions over the existing `/api/query-stream` path (no new REST routes, no second copy of the rule store). Rules stand until removed (no expiry): the reason and `created_by` are mandatory, the rejection message names the rule and the `remove_query_denial` call that lifts it, and `last_hit_at` (flushed every `MICROMEGAS_QUERY_DENY_REFRESH_SECONDS`, default 10s, which is also the cross-replica propagation bound) tells an operator whether a rule is still firing or safe to remove. An anti-jam escape hatch exempts any statement naming `deny_queries`/`remove_query_denial`/`list_query_denials` from a caller who could reach those functions anyway, so a rule can never lock out its own recovery path. A denial surfaces at three volumes: a `warn!` log line, a per-rule-tagged `query_denied` metric, and a new `error_class: "denied"` audit-log value (a `ResourceExhausted` status, chosen because it's the only existing code that already means "refused to spend resources here" without tripping the internal-error alerting path). Fail-open throughout: a refresh that can't reach Postgres keeps the previous snapshot, and a rule a given replica can't compile is dropped from that replica's snapshot alone rather than enforced blindly or treated as fatal -- this is an availability valve, not an authorization control. New knobs `MICROMEGAS_QUERY_DENY_REFRESH_SECONDS` (default 10) and `MICROMEGAS_QUERY_DENY_MAX_RULES` (default 100, bounding per-query evaluation cost). **Minor breaking change**: `CallerContext` (published, struct-literal-constructed, `micromegas_analytics::lakehouse::read_scope`) gains a new field, `identity: Option<String>`, so every downstream construction site needs updating; `QueryAuditRecord` (published, `micromegas::servers::query_audit`) gains a new field, `sql_hash: String`, appended last so existing JSON consumers are unaffected. Also fixes `LogStreamTableProvider` (backing every mutating admin UDTF) ignoring DataFusion's pushed-down column projection, which made any projected read of those functions -- e.g. `SELECT msg AS rule_id FROM deny_queries(...)`, or `SELECT msg FROM retire_partitions(...)` -- fail at physical planning; only `SELECT *` worked before.
* **Tracing:**
...
* **Telemetry Sink:**
  * Every micromegas process now accepts arbitrary process properties from the environment: `MICROMEGAS_PROCESS_PROPERTIES`, a comma-separated `key=value` list (e.g. `cluster=prod,role=cache`), is read once by `TelemetryGuardBuilder::build()` and merged into `ProcessInfo.properties`, so deployment tags land on the `processes` row and are queryable through the `process_properties` column every event view already exposes (`property_get(process_properties, 'cluster')`). Read by every binary built on the sink — the services, the monolith, `micromegas-redis-exporter`, and the C ABI — with no per-binary flag: `micromegas_main` builds the guard before `main`'s body runs and `ProcessInfo` is sent once at startup, so a CLI argument could not reach this without each binary hand-rolling its own `main`. Precedence is *explicit `with_process_property()` calls > environment > derived defaults* (the merge runs ahead of `populate_default_system_properties`, and both use `or_insert`), so an operator can add tags but cannot spoof the `version` `micromegas_main` stamps nor the host-derived `exe`/`username`/`computer`/`distro`/`cpu_brand` that dashboards filter on; within one list the first occurrence of a key wins. Entries are trimmed and blank ones dropped (a k8s manifest rendering an unset optional var produces `""`, and hand-written lists carry trailing commas), while a malformed entry — no `=`, an empty key, or a key in the server-written `micromegas.` namespace that ingestion would silently strip — fails `build()` before any telemetry flows, with `properties` left untouched rather than half-applied. Prefer this over a per-metric property for anything constant per process: a process property is stored once on the `processes` row, where the equivalent metric property is carried on every measure. See `mkdocs/docs/admin/telemetry-sink-tuning.md`. **Also fixes a latent silent failure this variable's fail-fast behavior exposed**: `micromegas_main`'s generated `main` bound `build()`'s `Result` to `let _telemetry_guard` without unwrapping it, so a failed telemetry init would leave the process running with no telemetry, no local sink, and nothing on stderr to say why. In practice no reachable failure existed before this change — a missing telemetry URL or unset auth env is not an error (the builder omits the HTTP sink and installs a no-op decorator) — so a malformed `MICROMEGAS_PROCESS_PROPERTIES` is the first `build()` rejection a macro-using binary can hit. The generated code now `expect`s it, so any guard-build failure panics with anyhow's full context chain and a non-zero exit instead of degrading invisibly. **Minor breaking change**: `micromegas_telemetry::property`'s `RESERVED_PROPERTY_PREFIX` and `PROPERTY_AUDIENCE` move to a new, ungated `micromegas_telemetry::property_names` module and are re-exported from `property`, so server-side `use` paths keep working; the point of the move is that `property` is gated behind the `server` feature (its `Property` type carries sqlx impls), and a client crate must be able to honor the reserved namespace without pulling in a database driver.
* **Auth:**
  * Add a DB-backed audience grant store (#1489, AbAC Stage 6a of the epic tracked at #1334): a new `audience_grants` table (data-lake schema v7) — `(audience, axis, selector)` as its own natural primary key, `axis IN ('read', 'mint')`, plus `created_at`/`created_by` (no `revoked_at`/`revoked_by`: a removed grant has no ongoing artifact whose provenance a caller might later need) — is what makes a per-user grant creatable without a service restart, unlike today's `{prefix}_AUDIENCE_GRANTS` env map, which is resolved once at startup. One table with an `axis` column rather than two, mirroring the env map's own single-map-for-both-axes shape and kept splittable later without touching `ReadPolicy`/`MintPolicy`. `AudienceGrants` gains `from_rows` (builds a grant map from `(audience, axis, selector)` triples, re-running the same `is_valid_audience`/selector-shape validation `parse` runs, so a row that slipped past the table's own `CHECK` constraints still can't reach a policy decision) and `merge` (unions two grant maps — a selector present in either source, or both, grants exactly the same access, with no dedup and no "duplicate" error); `merge` is a public utility exercised by its own integration tests (`rust/auth/tests/policy_tests.rs`), not a mechanism `resolve`/`resolve_audience` call — see below. A new `DbAudienceGrantsSource` (`micromegas_auth::db_audience_grants`) holds one whole-table snapshot per process (not a per-key `moka` cache — the issue is explicit the whole map is small enough to hold as one value), refreshed on a `{prefix}_AUDIENCE_GRANT_CACHE_TTL_SECONDS`-second TTL (default 60, falling back to the unprefixed name): the cold-start path (no snapshot yet) fails closed on a DB outage, throttled to one query attempt per TTL window; once any load has succeeded, a later refresh failure keeps serving the last good snapshot indefinitely instead — deliberately unbounded, unlike the DB-backed key store's per-key TTL-bounded staleness, because a store-wide fail-closed policy would turn a transient DB blip into "every caller loses every non-`public` audience simultaneously." A new `audience_grant_refresh_error_count` metric fires on every refresh failure. `AudienceReadPolicy`/`AudienceMintPolicy` gain `with_store(Option<Arc<DbAudienceGrantsSource>>)`; `resolve`/`resolve_audience` check the env map and the store's snapshot as two separate sources on every call — a selector present in either grants access — rather than merging them into one map, so neither side is deep-cloned per request; wired into `flight-sql-srv`'s and the monolith's flight-sql role's `AudienceReadPolicy` construction (one shared snapshot cache per process). `AudienceMintPolicy::with_store` exists and is unit-tested but has no production call site yet — `mint_key` still resolves audiences its own way, unchanged; wiring it is Stage 6/#1374's job. **Deploy-order requirement**: a standalone `flight-sql-srv` builds its `LakehouseContext` via `LakehouseContext::from_env()`, which only ever runs `migrate_lakehouse`, never the `migrate_db` migration (ingestion/monolith-only) that creates `audience_grants` at schema v7 — roll the v7 migration (via ingestion or the monolith) before or in the same deploy as this change reaches `flight-sql-srv`, or a still-v6 database makes every `resolve()` call fail with "relation audience_grants does not exist" (throttled to one attempt per cache-TTL window) until the migration catches up. `valid_selector` is now `pub`, so the new admin route below can run the exact same selector-shape check `AudienceGrants` itself uses. See `mkdocs/docs/admin/authentication.md`.
* **Web App:**
  * Add `analytics-web-srv` routes for the new audience grant store (#1489; self-service write policy #1510, AbAC Stage 6c): `POST`/`DELETE {base_path}/api/audience-grants`, gated by a new `GrantGate` extractor rather than a flat `AdminUser` — an admin acts unconditionally; a non-admin is admitted only once `MICROMEGAS_SELF_SERVICE_MINT` is on, and then further constrained per call: `POST` requires an identity selector (`user:`/`group:`, never `*`) and that the caller already hold `(audience, axis)` via an identity selector (delegation is per axis — a `read` grant lets you share `read`, a `mint` grant lets you share `mint`, never the other); `DELETE` requires the row be the caller's own direct `user:<email>` row or one they themselves created — except a non-admin's own `mint`/`user:<email>` row, which stays undeletable by them: it's the self-service claim marker `MICROMEGAS_SELF_SERVICE_MAX_CLAIMS_PER_CALLER` counts from (below), and deleting it would let a caller dodge that bound by claiming, deleting, and re-claiming. Both routes stay structurally unreachable (not just gated) under `--disable-auth` via the same `key_management_disabled_router` mechanism. `POST` upserts via a single `INSERT ... ON CONFLICT DO NOTHING` CTE unioned with a fallback `SELECT` (201 on a fresh create, 200 reporting the pre-existing row, with one automatic retry on the rare zero-row race between two concurrent creators); `DELETE` takes the natural `(audience, axis, selector)` key as query parameters, not path segments, since a `group:<id>` selector's charset isn't restricted enough to be a safe raw path segment. The paginated `GET {base_path}/api/audience-grants` (listing arbitrary rows) never ships — replaced by a new `GET {base_path}/api/audience-grants/visible`, the small unpaginated read backing the new Audience Access page's own list (below), which narrows to the caller's own rows when the self-service knob is off. Listing arbitrary rows from SQL now goes through a new caller-scoped `list_audience_grants()` table function instead — see the **Analytics** entry below. A new **Audience Access** page (`/audiences`), open to every authenticated user (not under `/admin`), lets a caller see what they can read/mint and why, share a pair they hold, remove their own access, revoke a share they created, and mint/claim an ingestion key — reachable from a header user-menu item (everyone) and an eighth Admin card (admins). See `mkdocs/docs/admin/web-app.md`, `mkdocs/docs/admin/authentication.md`, and `mkdocs/docs/admin/api-keys.md`. **Minor breaking change**: `analytics_web_srv::web_server::build_protected_routes` (published) replaces its separate `AnalyticsKeysState`/`IngestionKeysState` parameters with a single new `AdminRoutesState` struct that bundles those two plus the new `AudienceGrantsState`; `AudienceGrantError` (published, re-exported through `pub mod audience_grants`, not `#[non_exhaustive]`) gains an `Unauthenticated` variant and a matching `From<Unauthenticated>` impl, so any downstream exhaustive `match` needs a new arm. The `GrantGate`/`AdminUser`-to-self-service-policy gate widening itself is not a breaking-change entry — `create_grant`/`delete_grant` are private handlers and `GrantGate`, like `MintGate`, is a private extractor, so nothing else published changes shape.
* **Analytics:**
  * Add `list_audience_grants()`, a caller-scoped table function over the `audience_grants` DB store (#1510, AbAC Stage 6b of the epic tracked at #1334), registered for **every** authenticated caller — unlike the other lakehouse table functions gated on `caller.is_admin`, this one is never admin-only. Columns `audience`, `axis`, `selector`, `created_at`, `created_by`, ordered `audience, axis, selector`; no arguments, filter with `WHERE`. An admin sees every row; a non-admin sees every grant on each `(audience, axis)` pair they hold a matching grant on (deliberately wider than "rows whose selector matches me" — the same set they may also modify via `POST`/`DELETE /api/audience-grants`); a non-admin caller with no email and no groups (e.g. `CallerContext::internal()`) sees zero rows — a maintenance caller and a `--disable-auth` request are both treated as admin and see every row. Reads the table directly on every call, never a cached snapshot, so a write is visible on the next call. Deliberately does **not** apply `analytics-web-srv`'s `MICROMEGAS_SELF_SERVICE_MINT` knob the way the new `GET .../audience-grants/visible` REST route does — this function runs in a different crate/service with no visibility into that config, so it always applies the wider held-pair rule for a non-admin; an accepted, intentional asymmetry between the two read paths, not a bug. Backed by a new `grant_selectors: Arc<[String]>` field on `CallerContext`, precomputed by `rust/public` from the caller's `AuthContext` via a new `pub fn caller_selectors(&AuthContext) -> Vec<String>` (`micromegas_auth::policy`, also now used by `my_audiences`) so `micromegas-analytics` itself never needs to depend on `micromegas-auth`. See `mkdocs/docs/admin/functions-reference.md` and `mkdocs/docs/query-guide/functions-reference.md`. **Minor breaking change**: `CallerContext` (published, struct-literal-constructed, `micromegas_analytics::lakehouse::read_scope`) gains a new field, `grant_selectors: Arc<[String]>`, so every downstream construction site needs updating.
* **Python:**
  * Add the `micromegas-grants` CLI (`create`/`delete` subcommands) and two matching `WebClient` methods (`create_audience_grant`/`delete_audience_grant`), talking to the new `/api/audience-grants` routes over HTTP — never direct Postgres access, the same convention every CLI in this codebase follows (#1489). Listing goes through `micromegas-query --all "SELECT * FROM list_audience_grants()"` instead (#1510, see the **Analytics** entry above) — a non-admin gets their own scoped view, an admin gets every row with `WHERE`/`ORDER BY` to work with — so there is no `list` subcommand and no `WebClient.list_audience_grants`.
* **Services:**
  * New `micromegas-redis-exporter` binary and `micromegas-redis-exporter` Docker image: samples a single Redis instance over one persistent connection (default every 1s; cumulative presets `core`/`extended`/`full`, default `full`) and emits `redis_*` metrics through the standard telemetry sink, tagged with `instance=<target>` plus arbitrary `--property k=v` pairs; opt-in `--health-listen-addr` serves `/health` + `/ready` for Kubernetes probes.
* **Web App:**
  * `POST {base_path}/api/ingestion-api-keys` (mint) is no longer purely `AdminUser`-gated (#1374, AbAC Stage 6 of the epic tracked at #1334): a non-admin caller with a matching `mint` grant can now mint their own ingestion key, with `MintPolicy::resolve_audience` (`AudienceMintPolicy`, Stage 1/#1369, Stage 4/#1372) as the authorization instead of an admin gate — its first production call site. Resolved with a fresh, uncached, per-request point query against `audience_grants` (never a `DbAudienceGrantsSource` snapshot), deliberately asymmetric with the read side: mint grants are DB-only in this stage, so a mint selector declared only in `{prefix}_AUDIENCE_GRANTS` is inert. A non-admin caller who names a brand-new, never-before-granted audience *explicitly* claims it atomically as part of the same mint request — writing `user:<email>` grant rows on both the `mint` and `read` axes inside the transaction that also inserts the key — rather than being denied for lack of a pre-existing grant; naming an audience that already carries any grant row (or any `ingestion_api_keys` row) still requires a matching grant exactly as before. The claim takes a non-blocking, per-audience Postgres advisory lock (`pg_try_advisory_xact_lock`), so two concurrent claims for the same fresh name never both succeed: the loser gets a `409 CLAIM_CONTENDED`, not a 500 or a duplicate-owner state. Both the grant-based mint path and the lazy claim are off by default behind a new `MICROMEGAS_SELF_SERVICE_MINT` knob, plus two per-caller bounds, `MICROMEGAS_SELF_SERVICE_MAX_CLAIMS_PER_CALLER` (default 25) and `MICROMEGAS_SELF_SERVICE_MAX_KEYS_PER_CALLER` (default 100) — best-effort backstops against a runaway/abusive caller, not routine-use quotas — so an existing deployment's authorization surface never silently widens on upgrade. An admin caller minting into a brand-new audience is now claimed the same way (#1510, AbAC Stage 6c): the route runs the same ownership check as its own pre-check outside any lock, and on an unclaimed audience writes the admin's own `user:<email>` `mint`+`read` rows in the same transaction as the key insert — best-effort, so a concurrent claim losing the race never turns into a mint failure for the admin. `MintResponse` gains `claimed: bool`, `true` only when this call actually wrote those first grant rows; this is exactly what `micromegas-setup-telemetry`'s admin branch used to do client-side, now decided and performed server-side instead. `list_keys`/`revoke_key`/`import_key` stay `AdminUser`-gated and unchanged. A new `AuthenticatedUser` extractor (any authenticated caller, no admin check) backs the mint route's new `MintGate` and the new `GET {base_path}/api/audience-grants/my-audiences` route — caller-scoped, not admin-gated, answering only "which audiences may *I* mint into today," plus the caller's own `is_admin` flag, their caller-derived mint-namespace prefix, their own email (none otherwise exposed to a Bearer-authenticated CLI caller), and `held_pairs` — the `"{audience}:{axis}"` pairs the caller holds via an identity selector, always empty for an admin, which is how the Audience Access page tells which pairs it may offer to Share. `--disable-auth` gains a matching hardcoded `AuthContext` layer, purely as defensive parity — the key-management routers are never merged under `--disable-auth` in the first place, so self-service mint remains structurally unreachable there either way. See `mkdocs/docs/admin/authentication.md` and `mkdocs/docs/admin/api-keys.md`. **Minor breaking change**: `IngestionKeysState` (published, `analytics_web_srv::ingestion_keys`) gains three required fields — `self_service_mint_enabled: bool`, `max_claims_per_caller: i64`, `max_keys_per_caller: i64` — and `AudienceGrantsState` (published, `analytics_web_srv::audience_grants`) gains two required fields, `self_service_mint_enabled: bool` and `max_grants_per_caller: i64` (`MICROMEGAS_SELF_SERVICE_MAX_GRANTS_PER_CALLER`, default 50 — caps how many rows one non-admin caller may have created in `audience_grants`, counted across every audience/axis/selector but excluding the caller's own `user:<email>` rows, since those are claim/self-access rows rather than shares).
* **Python:**
  * Add the `micromegas-setup-telemetry` CLI (#1374, AbAC Stage 6): does an OIDC interactive login (reusing the existing loopback-redirect flow — no device-code flow, since none exists in this repo and the loopback flow already covers the developer/operator workstation target), mints a personal ingestion key via the new self-service mint route, and prints the `OTEL_EXPORTER_OTLP_PROTOCOL`/`_ENDPOINT`/`_HEADERS` exports needed to point a user's own telemetry at the deployment (or writes them to a `0o600` file with `--env-file`). Resolves `--audience` against `GET .../audience-grants/my-audiences`: an audience the caller already has a grant for is used verbatim; a fresh, non-admin claim is minted under a namespace derived from the caller's own email (printed to stderr) rather than the bare name typed, so operationally meaningful bare names stay out of self-service reach; an admin caller is never prefixed. An admin minting a brand-new audience now has it claimed for them server-side, as part of the mint route's own request (#1510, AbAC Stage 6c — see the **Web App** entry above): the mint response's new `claimed` field says whether this call actually wrote the audience's first grant rows, and the script prints `claimed audience <name>` to stderr when it did, rather than inferring it and writing the grant client-side via the admin grants API itself. Also adds `WebClient.mint_ingestion_api_key` (retries once on a `409 CLAIM_CONTENDED` — transient lock contention from a concurrent claim, not a denial) and `WebClient.my_audiences`.
* **Analytics:**
  * Promote `audience` from a process property resolved at query time to a physical, non-nullable column materialized directly on the six global-instance views -- `blocks`, `processes`, `streams`, `log_entries`, `measures`, `log_stats` (#1482). Extracted exactly once per partition write, from Postgres, at the `blocks` view's materialization, then propagated structurally into every downstream view the same way `processes.properties` already does. The column is `Dictionary(Int32, Utf8)` on all six views -- one distinct value per partition in practice, matching the `process_id` encoding in those same schemas -- so its type is identical wherever it appears. `OwnershipRewrite` (Prong A) filters those six views with a direct `audience IN (...)` predicate on their own column instead of a `process_id IN (subquery)` semi-join against a `property_get`-resolved aggregate -- the semi-join/`property_get` extraction disappears from every query plan touching them, and the column travels with the rows it governs instead of depending on a separate `processes` view materialization pass to catch up. The column can be non-nullable because this same change closes the last source of processes with no audience: a new `MICROMEGAS_DEFAULT_AUDIENCE` knob (default `public`) is stamped onto any process whose credential carries no audience, and an idempotent backfill, run at every ingestion-service startup, stamps the same value onto legacy rows that were never stamped -- see the amended **Ingestion** entry above. With unstamped gone as a state, the read-side `MICROMEGAS_UNSTAMPED_AUDIENCE` knob and Prong B's `OwnerAudience::Unstamped` variant are removed -- see the amended entries above. `net_spans`, `otel_spans`, `images`, `async_events`, and `thread_spans` are out of scope for this change and keep today's semi-join/`EXISTS` enforcement shapes, resolving through `__processes__partitions` as before.
  * **Operational note**: all six views bump their file-schema hash (`blocks_file_schema_hash()` `vec![3]` → `vec![5]`; `log_entries`/`measures` `SCHEMA_VERSION` 6/7 → 7/8; `processes`/`streams`/`log_stats` bump automatically with their inferred schema), making every pre-existing partition invisible until regenerated -- nothing is permanently lost, since lakehouse partitions already expire at the same retention horizon as their Postgres sources. Regenerate in dependency order: `regenerate_partitions('blocks', ...)` first (day-aligned `86400`-second buckets, RFC 3339 bounds -- midnight UTC, 90 days back to 2 days back, or whatever the deployment's retention window is), then `processes`/`streams`/`log_entries`/`measures`, then retire the old-hash `log_stats` partitions and rebuild via `materialize_partitions('log_stats', begin, end, <small delta, e.g. 3600>)` -- a day-sized bucket would force `log_stats`' extract query to sort a full day of aggregated output in one shot (see `tasks/completed/1392_kway_merge_sorted_partitions_plan.md`). The maintenance daemon covers the trailing window within one daily cycle on its own; until each view's regeneration completes, queries against it return post-deploy data only -- a visible gap, never a leak. Ingestion backfills `micromegas.audience` onto never-stamped processes with `MICROMEGAS_DEFAULT_AUDIENCE` at every startup -- set it before upgrading if `public` is not the label legacy data should carry; after a rolling upgrade, restart one ingestion replica if the maintenance log reports a `blocks` write rejected for a null `audience`. Lakehouse replication (`bulk_ingest`/`ingest_processes`) now rejects a source process with no `micromegas.audience` property -- upgrade the source lake first. `list_partitions` shows `'global'` partition rows to audience-scoped callers for view sets on `MICROMEGAS_PUBLIC_VIEW_SETS` or when the caller passes the lakehouse admin gate (previously: whenever the unstamped audience was in scope).
  * Collapse the two default-audience knobs into one, `MICROMEGAS_DEFAULT_AUDIENCE` (#1482, still `## Unreleased` -- neither name it replaces has shipped in a release under its old meaning). `MICROMEGAS_DEFAULT_INGESTION_AUDIENCE` (what audience *data written without one* gets, `rust/ingestion`) and `MICROMEGAS_DEFAULT_KEY_AUDIENCE` (what audience a *newly minted key* gets, `analytics-web-srv`) answered the same question about two doorways into the same deployment, and keeping them apart only ever made an operator configure the same label twice -- or, worse, configure one and forget the other. One knob now covers both, default `public`, resolved by `micromegas_auth::policy::default_audience_from_env` (still honouring the `{prefix}_DEFAULT_AUDIENCE` form) for the key routes and by `WriteAudience::default_from_env` (unprefixed) on the ingestion side. **Two behavior changes fall out of it.** (1) The knob no longer has an "unset" state to represent, so an empty or whitespace-only value is a startup error instead of meaning "not configured" -- an operator who wants writes to land somewhere unreadable sets a label no principal is granted (e.g. `unassigned`), which is also the fail-closed recipe the admin docs now give. (An *unset* variable still resolves to `public` on both sides, and a whitespace-padded one is trimmed and `warn!`-logged by both resolvers -- identical handling, so one env value can never be accepted by the web role and rejected by the ingestion role.) (2) Because the default always resolves, `POST .../ingestion-api-keys` with no `audience` field no longer returns **400**: it mints for the deployment default, the same value a credential with no bound audience stamps its data with. That drops a deliberate fail-closed check -- the old 400 existed so a fresh write credential could never silently become universally readable -- so a deployment that relied on it should set `MICROMEGAS_DEFAULT_AUDIENCE` to an ungranted label, and name the audience explicitly on every key it mints. The authorization decision is untouched: `MintPolicy::resolve_audience` still requires a matching `mint` grant (or a lazy claim) for whatever audience resolves, so the default decides which audience is asked about, never whether the caller may have it. **Minor breaking change**: `micromegas_auth::policy::default_key_audience_from_env` is renamed `default_audience_from_env` and returns `Result<String>` instead of `Result<Option<String>>`; `IngestionKeysState.default_audience` (published, struct-literal-constructed) is `String`, not `Option<String>`; `analytics_web_srv::ingestion_keys::resolve_audience` drops its per-route `fallback` parameter; and `micromegas_ingestion::write_audience::DEFAULT_INGESTION_AUDIENCE` is renamed `DEFAULT_AUDIENCE`.
  * A deployment built from `main` with `MICROMEGAS_UNSTAMPED_AUDIENCE` (or `{prefix}_UNSTAMPED_AUDIENCE`) still set fails at startup, naming `MICROMEGAS_DEFAULT_AUDIENCE` as its replacement -- the knob was never in a release, so there is no deprecation window.
  * **Minor breaking change**: `ProcessMetadata` (published, `micromegas_analytics::metadata`, all-public-fields) gains a required `audience: Arc<str>` field; `log_table_schema()`/`metrics_table_schema()` each gain a trailing dictionary-encoded `audience` column; `write_rows_and_track_times` (published, `micromegas_analytics::lakehouse::write_partition`) gains a required `&Schema` parameter, used by a new guard that fails a partition write naming the column when a batch carries a `NULL` under a declared non-nullable field (previously written silently as `""`); `blocks_view_schema()`'s `processes.parent_process_id` field is now declared nullable (it always could be `NULL` in practice; the DDL never enforced it) -- free alongside this change's hash bump. `IsolationConfig` (published, `micromegas_analytics::lakehouse::read_scope`) drops its `unstamped_audience` field; `OwnershipRewrite::new` and `AudienceGuard::new` (published) each drop their `unstamped_audience` parameter, `AudienceGuard::new` gaining a `lakehouse_admin: bool` in its place.
