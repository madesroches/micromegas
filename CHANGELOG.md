...
* **Build:**
  * Bump `google.golang.org/grpc` to `1.82.1` in the grafana plugin's Go module to resolve Dependabot alert #381
  * Bump `dompurify` to `^3.4.12` and `fast-uri` to `^3.1.4` via yarn `resolutions`/`overrides` across the root, `doc/intro-micromegas`, and `doc/high-frequency-observability` workspaces to resolve Dependabot alerts #383, #380, #379, #384, #382
  * Upgrade `react-router-dom` to `^7.18.0` in `analytics-web-app`, and force the transitive `react-router` resolution to `^7.18.0` in the root workspace (pulled in only via `@grafana/ui`'s legacy v5 routing compat shim), to resolve Dependabot alerts #386, #385, #378, #377, #376 — no fix exists in the v6 line for these advisories
