...
function IngestionApiKeysPageContent() {
  ...
  const [revokeTarget, setRevokeTarget] = useState<IngestionApiKeyListEntry | null>(null)
  ...
  const handleMint = async () => {
    ...
    setMintError(null)
    try {
      const result = await mintIngestionApiKey(mintName.trim())
      ...
      setMintError(err instanceof IngestionApiKeyError ? err.message : 'Failed to mint key')
    } finally {
      setIsMinting(false)
    }
  }
  ...
  const handleRevoke = async () => {
    ...
    setIsRevoking(true)
    try {
      await revokeIngestionApiKey(revokeTarget.key_id)
      setRevokeTarget(null)
      await loadKeys()
    } catch (err) {
      setError(err instanceof IngestionApiKeyError ? err.message : 'Failed to revoke key')
      setRevokeTarget(null)
    } finally {
      setIsRevoking(false)
    }
      ...
      <PageLayout onRefresh={loadKeys}>
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="mt-1 text-theme-text-secondary">
                Manage write credentials for telemetry ingestion clients.
              </p>
            </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="relative w-full max-w-md bg-app-panel border border-theme-border rounded-lg shadow-xl">
                <div className="px-4 py-3 border-b border-theme-border">
                  <h2 className="text-lg font-medium text-theme-text-primary">Mint Ingestion API Key</h2>
                </div>
                <div className="p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. game-client-42"
                      value={mintName}
          ...
          <ConfirmDialog
            ...
            message={`Are you sure you want to revoke "${revokeTarget?.name}"? Any client using this key will lose access once the revocation propagates.`}
            confirmLabel="Revoke"
