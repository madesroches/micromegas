...
function AnalyticsApiKeysPageContent() {
  ...
  const loadKeys = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await listAnalyticsApiKeys(true, offset)
      setKeys(data)
    } catch (err) {
  ...
  const goToPreviousPage = () => {
    setOffset((current) => Math.max(0, current - MAX_ANALYTICS_API_KEYS_LIST_LIMIT))
  }

  const goToNextPage = () => {
    setOffset((current) => current + MAX_ANALYTICS_API_KEYS_LIST_LIMIT)
  }
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          {isLoading ? (
            ...
            <div className="border border-theme-border rounded-lg overflow-hidden">
              ...
              {(offset > 0 || keys.length === MAX_ANALYTICS_API_KEYS_LIST_LIMIT) && (
                <div className="flex items-center justify-between border-t border-theme-border bg-app-panel px-4 py-2.5">
                  ...
                  {keys.length === MAX_ANALYTICS_API_KEYS_LIST_LIMIT ? (
