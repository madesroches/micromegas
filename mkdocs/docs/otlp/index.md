...
process_id = uuid_v5(NS_OTEL_PROCESS_V1, key)                                   # resolved write audience == deployment default
process_id = uuid_v5(uuid_v5(NS_OTEL_PROCESS_V1, write_audience), key)          # resolved write audience != deployment default
```
...
- **`block_id` is hashed from the *full* incoming header set, not just the 3 recognized
  ...
  formula) — the write audience, whenever it differs from the deployment default, still
