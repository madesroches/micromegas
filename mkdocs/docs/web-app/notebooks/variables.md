...
- **Errors**: an unparseable override surfaces as the cell's error state, the same as a bad SQL query. Does not apply when the cell's data source resolves to `notebook` — see below.
- **Waiting for selection**: if the override references `$cell.selected.column` and no row is selected yet, the cell shows the same "waiting for selection" placeholder described under [Row Selection](#row-selection) above. Does not apply when the cell's data source resolves to `notebook` — see below.
- Set it in the cell editor via the **Query Time Range** field, shown below the data source selector for every cell type that supports it — except a cell whose data source resolves to `notebook` (see below). It is also offered for horizontal-group children whose query runs server-side, hidden there under the same rule.
- **Notebook (WASM) cells have no per-cell time range.** When a cell's data source resolves to `notebook` (local WASM execution), the **Query Time Range** field is not shown, and any override already saved on the cell is ignored entirely: `$from`/`$to` inside the cell's SQL, the display axis (Swimlane, Property Timeline), and Map playback all follow the screen's global range unconditionally. This is because a WASM-registered table is plain Arrow batches with no designated time column to bound a `begin`/`end` against — there's no column-agnostic way to enforce it the way a server-side view does. To narrow what a notebook-source cell sees, filter with SQL (`WHERE`) or narrow the range of the upstream cell whose result it queries.
...
The accepted unit vocabulary is the same set the chart understands — see `lib/units.ts` for canonical names and aliases (`bytes`, `KB`, `MB`, `seconds`, `ms`, `µs`, `bits/s`, `percent`, `degrees`, `boolean`, ISO 4217 currency codes like `USD`/`CAD`/`EUR`, …). This also includes the UCUM/OTLP unit codes emitted by CloudWatch Metric Streams and other OTLP producers (`By`, `KiBy`, `MBy/s`, `GBit/s`, …) and dimensionless forms (`1`, `{Count}`, `none`, `count`), which render as a bare number with no unit suffix.
