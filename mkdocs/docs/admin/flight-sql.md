...
| `MICROMEGAS_AUDIENCE_GRANTS` | No | **Deprecated**, superseded by the `audience_grants` table — a JSON grant map keyed by audience name, still unioned in on the read axis where set. See [Deprecated: the env grant map](authorization.md#deprecated-the-env-grant-map) |
