...
| `MICROMEGAS_ANALYTICS_USER_MAINTENANCE_FUNCTIONS` | No | `true`/`false` (case-insensitive, default `false`; falls back to unprefixed `MICROMEGAS_USER_MAINTENANCE_FUNCTIONS`). Registers the five mutating lakehouse UDTFs/UDFs for *every* authenticated caller, not just an admin — the way back for an API-key-only deployment, which otherwise has no admin principal (an API key can never be admin) and so no access to these functions at all. **Deployment-wide, not per-audience**: enabling this grants any authenticated caller destructive access to every audience's partitions — safe only when no admin principal exists, unsafe the moment the deployment also has personal or per-team audiences |
