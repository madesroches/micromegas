...
| `MICROMEGAS_ANALYTICS_IMPLICIT_GROUPS` | No | Comma-separated groups every authenticated FlightSQL caller is implicitly a member of, for audience resolution (falls back to unprefixed `MICROMEGAS_IMPLICIT_GROUPS`) |
| `MICROMEGAS_ANALYTICS_UNSTAMPED_AUDIENCE` | No | Audience (`user:<id>`/`group:<id>`) a process with no `micromegas.audience` property falls back to for `OwnershipRewrite`'s query-time audience filtering (falls back to unprefixed `MICROMEGAS_UNSTAMPED_AUDIENCE`); unset means such processes stay invisible to every restricted caller. **Required, together with `MICROMEGAS_ANALYTICS_IMPLICIT_GROUPS=everyone`, to keep legacy data visible when enabling auth on a deployment with unstamped data** — see the CHANGELOG's AbAC Stage 2 upgrade note (a dedicated isolation-activation page is planned for a later stage). **This pair is not sufficient by itself**: `OwnershipRewrite` resolves a process's audience from the *materialized* `processes`/`streams` views, which only the maintenance role's periodic pass populates — a process with no row there yet (including one just ingested) stays invisible to every restricted caller regardless of this knob until that pass catches up |
