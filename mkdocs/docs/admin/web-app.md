...
# REQUIRED whenever auth is enabled (startup error naming it if unset) --
# without it no session can resolve admin-ness or group grants at all, since
# admin-ness itself now comes from the `admins` local group (see
# mkdocs/docs/admin/groups.md), resolved from this same connection. Backs
# ALL FOUR route groups -- analytics-web-srv is the sole admin HTTP surface
# for ingestion_api_keys, analytics_api_keys, audience_grants, and
