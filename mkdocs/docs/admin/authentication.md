...
Micromegas supports unified authentication across all services using both API
keys and OpenID Connect (OIDC). This page covers *identity* only — what a
caller may then read and write is [Authorization](authorization.md).
...
Authentication establishes *who* is calling; authorization decides *what* they
may read and write. Configuring any authentication method activates the
authorization layer: queries are narrowed to the audiences the caller is
granted, and a few lakehouse-management functions are gated on admin-ness.

That model — audience labels, the grant map, write-time stamping, self-service
key mint — is documented separately in [Authorization](authorization.md).
...
Minting an ingestion key, or reading an audience's data, is always a grant row
