...
| `process_resident_bytes` / `process_virtual_bytes` | This process's own RSS/virtual size (`sysinfo`, allocator-agnostic; emitted by any service built with the `jemalloc-metrics` feature). Process-wide, not object-cache-specific. |
