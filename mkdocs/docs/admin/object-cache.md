...
| `MICROMEGAS_OBJECT_CACHE_FETCH_MEMORY_BUDGET_MB` | No | Cap on transient origin-GET buffer memory across all in-flight coalesced fetch runs, in MiB (default `256`: the smallest budget at which the default 32 concurrent fetches stay fully usable for max-size 8 MiB runs, so a cold contiguous scan is throttled by parallelism rather than memory); must be in `1..=1048576` (1 TiB), and at least `(DEMAND_RESERVED_FETCHES + 1) * max_run_bytes(BLOCK_SIZE, MAX_COALESCED_GET_BYTES)` or the server refuses to start |
...
| `range_cache_size_backend_hit` (`+ prefix`) | cache server | `size()` lookups served from the backend. Fires exactly once per ranged GET. |
...
| `object_cache_get_requests` (`status, prefix`) | cache server | Every `GET /obj/{key}` outcome — success and failure alike. Slice by `status` for the error-rate breakdown. |
...
| `process_resident_bytes` / `process_virtual_bytes` | This process's own RSS/virtual size (`sysinfo`, allocator-agnostic; emitted by any service with system metrics enabled, regardless of the `jemalloc-metrics` feature). Process-wide, not object-cache-specific. |
| `jemalloc_allocated_bytes` / `jemalloc_resident_bytes` / `jemalloc_mapped_bytes` / `jemalloc_retained_bytes` | jemalloc's own runtime accounting (`stats.allocated`/`resident`/`mapped`/`retained`); only emitted by binaries built with the `jemalloc-metrics` feature, all 8 production services. Process-wide, not object-cache-specific. |

Diagnosing a process-memory climb: compare `process_resident_bytes` against the host-level `used_memory` system metric — RSS climbing while the delta between `used_memory` and this process's RSS stays flat means the growth *is* this process; RSS flat while `used_memory` climbs means it's some other process on the host. Then, of the jemalloc gauges: `jemalloc_allocated_bytes` climbing points to a logical leak inside the process (reach for a jemalloc heap profile next); `resident` climbing while `allocated` is flat points to allocator fragmentation/retention — see [Memory Allocator Configuration](memory-allocator.md) for the decay tuning and the `_RJEM_MALLOC_CONF` override; process RSS climbing while both jemalloc gauges are flat points to non-heap growth (mmap, thread stacks, kernel).

Disk-tier bytes and disk-tier entry count are not emitted: surfacing foyer's live disk-tier occupancy would require adding `mixtrics` as a direct dependency and a custom metrics-registry adapter, so only the RAM-tier gauges above exist today.
