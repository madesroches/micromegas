...
| `MICROMEGAS_DEFAULT_AUDIENCE` | No | The audience a process whose ingestion credential carried no audience is **read** as (default `public`). Set it identically on every role that builds a lakehouse — [FlightSQL](flight-sql.md), this one, and the [monolith](monolith.md) — since **this** role is what bakes the value into partitions — a deployment that sets the knob only on the query role materializes under the wrong default. Changing it does not relabel already-written partitions; regenerate the six views over any range that should reflect the new value. See [Audience stamping and the default](authentication.md#audience-stamping-and-the-default) |
