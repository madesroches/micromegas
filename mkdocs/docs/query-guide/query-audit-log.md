...
| `client_ip` | string | always | Network-level truth, not self-reported attribution, for requests that actually traversed the trusted proxy: the rightmost `X-Forwarded-For` entry (the address the trusted proxy/ALB in front of this service observed), falling back to `X-Real-IP` and then the gRPC peer address for a direct connection. `unknown` if none is available. For a direct connection with no proxy in front, both `X-Forwarded-For` and `X-Real-IP` are caller-chosen. Matches the `client_ip` on `flight-sql-srv`'s generic per-call `uri=... client_ip=...` log line (both come from the same `get_client_ip`), so the two can be cross-referenced |
